...
let _teamUUID = ''
function tu() {
  if (!_teamUUID) _teamUUID = getTeamUUID()
  return _teamUUID
}
...
async function resolveProjectNames(reviews: any[]): Promise<Record<string, string>> {
  ...
  await Promise.all(
    projectUuids.map(async (puuid) => {
      try {
        // exchange API: 短标识 → 真实 UUID + identifier
        const exchRes = await fetch(
          `/project/api/ones-project/team/${tuid}/projects/exchange/${puuid}`,
          { credentials: 'include' },
        )
        ...
        >
          {filtered.map((t) => (
            <div
              ...
              onMouseDown={(e) => {
                e.preventDefault()
                onChange(t.scope_uuid, t.issue_type_uuid)
                setKw('')
                setOpen(false)
              }}
              style={{
...
const App: React.FC = () => {
  ...
  async function init() {
    ...
    try {
      ...
      if (meUuid) {
        ...
        const allReviews = [
          ...(data.review_pending || data.pending || []),
          ...(data.resolution_pending || []),
          ...(data.done || []),
        ]
        ...
        const enrich = (arr: any[]) =>
          arr.map((r) => ({
            ...r,
            project_name: projectNameMap[r.project_uuid] || r.project_name || r.project_uuid,
          }))
  ...
  async function openReview(rv: any, mode?: 'review' | 'resolution' | 'readonly') {
    ...
    // （提交意见/发布决议等动作由详情页「IPD评审操作」快捷按钮承载）
    if (rv?.issue_number) {
      try {
        const url = `/project/#/team/${tu()}/project/${rv.project_uuid || ''}/issue/${rv.issue_number}`
  ...
  if (msg && !selected)
    return <div style={{ ...S.container, textAlign: 'center', color: '#cf1322' }}>{msg}</div>
    ...
    <div style={S.container}>
      ...
      <div style={{ display: 'flex', gap: 0, borderBottom: '1px solid #e8e8e8', marginBottom: 16 }}>
        <button
          ...
          style={{
            ...
            cursor: 'pointer',
            fontSize: 14,
            borderBottom: myTab === 'pending' ? '2px solid #1677ff' : '2px solid transparent',
            color: myTab === 'pending' ? '#1677ff' : '#666',
            fontWeight: myTab === 'pending' ? 600 : 400,
          }}
        >
        ...
        <button
          ...
          style={{
            ...
            cursor: 'pointer',
            fontSize: 14,
            borderBottom: myTab === 'resolution' ? '2px solid #faad14' : '2px solid transparent',
            color: myTab === 'resolution' ? '#faad14' : '#666',
            fontWeight: myTab === 'resolution' ? 600 : 400,
          }}
        >
        ...
        <button
          ...
          style={{
            ...
            cursor: 'pointer',
            fontSize: 14,
            borderBottom: myTab === 'done' ? '2px solid #1677ff' : '2px solid transparent',
            color: myTab === 'done' ? '#1677ff' : '#666',
            fontWeight: myTab === 'done' ? 600 : 400,
          }}
        >
      ...
      {(() => {
          ...
          return (
            <div style={{ ...S.card, textAlign: 'center', padding: 40, color: '#999' }}>
              ...
              <div style={{ fontSize: 13 }}>
                {myTab === 'pending'
                  ? '当有评审单进入评审中状态后，将显示在这里'
                  : myTab === 'resolution'
                    ? '当前置评审人完成评审后，待决议项将显示在这里'
                    : '完成评审后，将显示在这里'}
              </div>
            </div>
          )
        ...
        const projectOptions = [
          ...new Set(displayReviews.map((r) => r.project_name || r.project_uuid).filter(Boolean)),
        ]
        ...
        return (
          <React.Fragment>
            ...
            {filteredReviews.length === 0 ? (
              <div
                style={{ ...S.card, textAlign: 'center', padding: 32, color: '#999', fontSize: 13 }}
              >
                无匹配结果
              </div>
            ) : (
                ...
                <div style={S.tableWrap}>
                  <table style={S.table}>
                    <thead>
                      <tr>
                        <th style={S.th}>编号</th>
                        <th style={{ ...S.th, width: 70, textAlign: 'center' }}>类型</th>
                        <th style={S.th}>阶段</th>
                        <th style={S.th}>标题</th>
                        <th style={{ ...S.th, width: 120 }}>项目</th>
                        <th style={{ ...S.th, width: 100, textAlign: 'center' }}>评审进度</th>
                        {myTab === 'resolution' && (
                          <th style={{ ...S.th, width: 90, textAlign: 'center' }}>操作</th>
                        )}
                        {myTab === 'done' && (
                          <th style={{ ...S.th, width: 80, textAlign: 'center' }}>我的结论</th>
                        )}
                      </tr>
                    </thead>
                    <tbody>
                        ...
                        >
                          ...
                          >
                            <span
                              ...
                              onClick={async (e) => {
                                e.stopPropagation()
                                await copyReviewLinkToClipboard(r.review_number || r.review_uuid)
                              }}
                            >
                              {r.review_number || '-'}
                            </span>
                          ...
                          <td style={{ ...S.td, textAlign: 'center' }}>
                            <span
                              style={{
                                ...
                                fontWeight: 600,
                                background:
                                  (r.review_type || 'dcp') === 'tr' ? '#f9f0ff' : '#e6f4ff',
                                color: (r.review_type || 'dcp') === 'tr' ? '#722ed1' : '#1677ff',
                              }}
                            >
                              {(r.review_type || 'dcp') === 'tr' ? 'TR' : 'DCP'}
                            </span>
                          </td>
                          ...
                          {myTab === 'resolution' && (
                            <td style={{ ...S.td, textAlign: 'center' }}>
                              <button
                                style={{ ...S.btn(true), fontSize: 12, padding: '2px 10px' }}
                                onClick={(e) => {
                                  e.stopPropagation()
                                  openReview(r, 'resolution')
                                }}
                              >
                                去发布决议
                              </button>
                          ...
                          {myTab === 'done' && (
                            <td style={{ ...S.td, textAlign: 'center', fontSize: 12 }}>
                              <span
                                style={{
                                  color:
                                    r.my_conclusion === 'pass'
                                      ? '#52c41a'
                                      : r.my_conclusion === 'fail'
                                        ? '#ff4d4f'
                                        : '#faad14',
                                }}
                              >
                                {r.my_conclusion === 'pass'
                                  ? ''
                                  : r.my_conclusion === 'conditional_pass'
                                    ? ''
                                    : ''}
                              </span>
                            </td>
...
const UserPicker: React.FC<{
  value: string
  onChange: (u: { uuid: string; name: string }) => void
  placeholder?: string
  displayName?: string
}> = ({ value, onChange, placeholder = '搜索用户…', displayName }) => {
  ...
  async function doSearch(k: string) {
    ...
    const filtered = members
      .filter(
        (u: any) =>
          u.name.toLowerCase().includes(kwLower) || u.email.toLowerCase().includes(kwLower),
      )
      .slice(0, 20)
    setResults(filtered)
  ...
  if (shownName) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
        <span style={{ fontSize: 12, background: '#e6f4ff', padding: '2px 8px', borderRadius: 3 }}>
          {shownName}
        </span>
        <button
          onClick={() => {
      ...
      {open && results.length > 0 && (
        ...
        >
          {results.map((u: any) => (
            ...
            >
              <span>{u.name}</span>
              <span style={{ color: '#999', fontSize: 11 }}>{u.email}</span>
            </div>
          ))}
        </div>
      )}
...
const ReviewerWorkspace: React.FC<{
  ...
  const alreadySubmitted = !!(
    myReviewer?.submitted_at > 0 && (myReviewer?.round_no || 1) === _wsRoundNo
  )
  ...
  useEffect(() => {
    callApi('/ipd/config')
      .then((c: any) => {
        ...
        setConfigRoles(
          (c.roles || []).filter((r: any) => (r.review_type || 'dcp') === _rvReviewType),
        )
  ...
  const remediationCreationBlocked =
    remediationTypeStatus === 'loading' ||
  ...
  const resolutionReady = (() => {
    ...
    const publisherRole =
      resolutionRule.publisher?.role ||
      (Array.isArray(resolutionRule.publisher?.roles) ? resolutionRule.publisher.roles[0] : '') ||
      ''
  ...
  const _prevResolution = (data.resolutions || []).find(
    (r: any) => (r.round_no || 1) === _wsRoundNo - 1,
  )
  const canPublishResolution =
    canPublish && rv.status === 'reviewing' && resolutionReady && !data.resolution
  ...
  useEffect(() => {
    ...
    const selectedType =
      findConfiguredIssueType(issueTypes, remediationIssueType, remediationIssueTypeUuid) ||
  ...
  async function resolveCreationIssueType() {
    const result = await resolveProjectIssueTypes(
      tu(),
      createIssueForm.project_uuid || rv.project_uuid || '',
    )
    setIssueTypes(result.types)
    setProjectTypesVerified(result.verified)
    if (!result.verified)
    ...
    if ((remediationIssueType || remediationIssueTypeUuid) && !configured) {
      throw new Error(
        remediationTypeBlockedMessage('missing', remediationIssueType || remediationIssueTypeUuid),
      )
    }
    const selected =
      configured ||
    ...
    // Checklist 前端预检：当前角色所有项必须已勾选
    {
      ...
      if (myChecklistItems.length > 0) {
        ...
        if (unchecked.length > 0) {
          ...
          setOpinionMsg(`请先完成所有 Checklist 勾选后再提交，未勾选：${names}`)
          return
        }
      }
    }
  ...
  async function syncRemediationFromBrowser(): Promise<boolean> {
    const remediationIssues = (data.remediation_issues || []).filter(
  ...
  async function handlePublishResolution() {
    if (!resolutionForm.final_conclusion) {
      setResolutionMsg('请选择决议结果')
      return
    }
    if (
      (resolutionForm.final_conclusion === 'conditional_pass' ||
        resolutionForm.final_conclusion === 'rework') &&
      !resolutionForm.condition_notes.trim()
    ) {
      setResolutionMsg('结论为「有条件通过」或「返工」时，必须填写条件说明')
      return
    }
    setResolving(true)
    setResolutionMsg('')
    try {
      ...
      setResolutionMsg(e.message || '发布失败')
    } finally {
      setResolving(false)
    }
  }
  ...
  function downloadMaterial(templateId: string) {
    callApi(`/ipd/review/${rv.review_uuid}/material/${templateId}/download-url`)
      .then((r: any) => {
        if (!r.url) return
  ...
  function downloadAttachment(objectKey: string) {
    callApi(
      `/ipd/review/${rv.review_uuid}/material-attachment/download-url?object_key=${encodeURIComponent(objectKey)}`,
    )
      .then((r: any) => {
        if (!r.url) return
        const a = document.createElement('a')
  ...
  async function previewAttachment(objectKey: string, fileName: string) {
    ...
    try {
      const r: any = await callApi(
        `/ipd/review/${rv.review_uuid}/material-attachment/preview?object_key=${encodeURIComponent(objectKey)}&file_name=${encodeURIComponent(fileName)}`,
      )
      if (r.content) {
        const previewable = [
          'application/pdf',
    ...
    try {
      ...
      if (add3Res.ok) {
        ...
        if (task?.uuid) {
          ...
          const resetType =
            remediationIssueType || remediationIssueTypeUuid
              ? resolvedType.issueType
              : issueTypes[0] || resolvedType.issueType
          setCreateIssueForm({
            title: '',
  ...
  return (
    <div style={S.container}>
      <div style={{ marginBottom: 16, display: 'flex', gap: 8 }}>
        ...
        {directLink && (
          <button
            style={{ ...S.btn(false), borderColor: '#1677ff', color: '#1677ff' }}
            onClick={() => {
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <span
              ...
              title="点击复制编号"
              onClick={() => handleCopyReviewLink(rv.review_number || rv.review_uuid)}
            >
              {rv.review_number || ''}
            </span>
            <strong style={{ fontSize: 16 }}>
              {rv.phase_name || rv.phase_code} — {rv.review_title || 'DCP评审'}
            </strong>
          </div>
          <div style={{ fontSize: 12, color: '#666' }}>
            项目: {projectDisplayName} | 创建:{' '}
                            ...
                            >
                              {attachments.map((att: any, ai: number) => (
                                ...
                                >
                                  ...
                                  <span
                                    style={{ cursor: 'pointer', textDecoration: 'underline' }}
                                    onClick={() => previewAttachment(att.file_data, att.file_name)}
                                  >
                                    {att.file_name}
                                  </span>
                                  <button
      ...
      <div style={S.card}>
        <h4 style={S.sectionTitle}>
          {canPublish ? '整改项' : '我的整改项'}（{(canPublish ? allIssues : myIssues).length}个）
        </h4>
        {!canPublish && allIssues.length > myIssues.length && (
          <div style={{ fontSize: 12, color: '#999', marginBottom: 8 }}>
            （共 {allIssues.length} 个关联工作项，仅显示你创建的 {myIssues.length} 个）
          </div>
        )}
        {(canPublish ? allIssues : myIssues).length === 0 ? (
          ...
          <table style={S.table}>
            <thead>
              <tr>
                ...
                <th style={S.th}>标题</th>
                <th style={{ ...S.th, width: 110 }}>工作项类型</th>
              ...
              {createIssueMsg && (
                <div
                  style={{
                    ...
                    borderRadius: 4,
                    fontSize: 12,
                    background: createIssueFallback ? '#fff7e6' : '#fff2f0',
                    color: createIssueFallback ? '#faad14' : '#cf1322',
                  }}
                >
                  {createIssueMsg}
                  {createIssueFallback && (
                        ...
                        onClick={(e) => {
                          ...
                          try {
                            window.parent.location.href =
                              window.parent.location.origin + '/' + createIssueFallback
                          } catch {}
                        }}
                        style={{
                          display: 'inline-block',
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap' }}>
                <div style={S.formGroup}>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setCreateIssueForm({ ...createIssueForm, title: e.target.value })
                    }
                    placeholder="工作项标题"
                    disabled={remediationCreationBlocked}
                  />
                </div>
                <div style={S.formGroup}>
                  ...
                  {loadingTypes ? (
                    ...
                    <input
                      ...
                      value={remediationIssueType || remediationIssueTypeUuid}
                      disabled
                    />
                  ) : (
                    // 未配置：允许选择
                    <SearchableTypePicker
                      ...
                      onChange={(scopeUuid, issueTypeUuid) =>
                        setCreateIssueForm({
                          ...createIssueForm,
                          issue_type_scope_uuid: scopeUuid,
                          issue_type_id: issueTypeUuid,
                        })
                      }
                    />
                  )}
                ...
                <div style={S.formGroup}>
                  ...
                  <UserPicker
                    value={createIssueForm.assignee_uuid}
                    onChange={(u) =>
                      setCreateIssueForm({ ...createIssueForm, assignee_uuid: u.uuid })
                    }
                    placeholder="搜索用户…（必填）"
                  />
                </div>
                <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                  <button
                    style={{ ...S.btn(true), marginBottom: 0 }}
                    onClick={handleCreateIssue}
                    disabled={creating || remediationCreationBlocked}
                  >
                    {creating ? '创建中…' : '创建整改项'}
                  </button>
                  <button
                    style={{ ...S.btn(false), marginBottom: 0 }}
                    onClick={resolveAndOpenCreateIssue}
                    disabled={resolvingProject || remediationCreationBlocked}
                  >
      ...
      {!isResolutionMode && (
        <div
          style={{
            ...S.card,
            background: alreadySubmitted ? '#f6ffed' : '#f0f5ff',
            borderLeft: `4px solid ${alreadySubmitted ? '#52c41a' : '#1677ff'}`,
          }}
        >
          ...
          {_isReReviewing && !alreadySubmitted && (
            ...
            >
              第 {_wsRoundNo} 轮复审
              {_prevResolution
                ? `（上一轮决议：${_prevResolution.final_conclusion === 'conditional_pass' ? '有条件通过' : _prevResolution.final_conclusion === 'rework' ? '返工' : _prevResolution.final_conclusion}）`
                : ''}
              。整改项已完成，请重新评估并提交评审意见。
            </div>
          )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <div style={S.formGroup}>
              ...
              <input
                style={{ ...S.input, background: '#f5f5f5', color: '#666' }}
                value={currentUser.name || currentUser.uuid || '（未识别）'}
                readOnly
              />
            </div>
            <div style={S.formGroup}>
            ...
            <div style={{ marginTop: 12 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <div style={S.formGroup}>
                  ...
                  <input
                    style={{ ...S.input, background: '#f5f5f5', color: '#333' }}
                    value={
                      opinionForm.conclusion === 'pass'
                        ? '✅ 通过'
                ...
                <div style={S.formGroup}>
                  ...
                  <input
                    style={{ ...S.input, background: '#f5f5f5', color: '#333' }}
                    value={
                      opinionForm.risk_level === 'low'
                        ? '低'
                        : opinionForm.risk_level === 'medium'
                          ? '中'
                          : '高'
                    }
                    readOnly
              ...
              <div style={{ marginTop: 8, fontSize: 12, color: '#52c41a' }}>
                提交时间:{' '}
                {myReviewer?.submitted_at
                  ? new Date(myReviewer.submitted_at).toLocaleString('zh-CN')
                  : ''}
              </div>
            </div>
          ) : (
            <>
              ...
              <div style={{ ...S.formGroup, marginTop: 8 }}>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setOpinionForm({ ...opinionForm, opinion_summary: e.target.value })
                  }
              ...
              <tbody>
                {reviewers.map((v: any, i: number) => (
                  <tr key={i}>
                    ...
                    <td style={S.td}>
                      {v.risk_level === 'low' ? '低' : v.risk_level === 'high' ? '高' : '中'}
                    </td>
                    <td
                      style={{
                        ...S.td,
      ...
      {(() => {
        ...
        const votes: any[] = res?.based_on_votes
          ? (() => {
              try {
        ...
        if (res) {
          ...
          return (
            <div style={{ ...S.card, background: '#f6ffed', borderLeft: '4px solid #52c41a' }}>
              <h4 style={S.sectionTitle}>
                决议快照 {res.snapshot_number ? `(${res.snapshot_number})` : ''}
              </h4>
              <div
                style={{
                  display: 'grid',
              ...
              >
                ...
                <div style={S.formGroup}>
                  ...
                  <span style={{ fontSize: 13 }}>
                    {res.published_by_name || res.published_by || '-'}
                  </span>
                </div>
              </div>
              {res.condition_notes && (
              ...
              {(isResolutionMode || votes.length > 0) && (
                <div>
                  <label style={S.label}>
                    {votes.length > 0 ? '评审意见汇总（投票快照）' : '评审意见汇总'}
                  </label>
                  <div style={S.tableWrap}>
                    <table style={S.table}>
                      <thead>
          ...
          return (
            <div style={{ ...S.card, background: '#fff7e6', borderLeft: '4px solid #faad14' }}>
                ...
                {(() => {
                  ...
                  if (submitMode === 'must_vote_roles') {
                    const mustVoteNames = configRoles
                      ...
                      .map((r: any) => r.role_name)
                    const mustRvrs = reviewers.filter((r: any) =>
              ...
              {(() => {
                ...
                const approvalConclusions = resolutionRule?.passRule?.approvalConclusions || [
                  'pass',
                  'conditional_pass',
                ]
                if (passMode === 'min_approval_count') {
                  ...
                  if (candidates.length < minCount) {
                    return (
                      ...
                      >
                        可计票评审人仅 {candidates.length} 人，规则要求至少 {minCount}{' '}
                        人通过，请补充评审人或调整规则
                      </div>
                    )
                  }
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <div style={S.formGroup}>
                  ...
                  >
                    ...
                    {(
                      resolutionRule?.allowedConclusions || ['pass', 'conditional_pass', 'reject']
                    ).map((c: string) => {
                      const labels: any = {
                        pass: '✅ 通过',
              ...
              <div style={{ ...S.formGroup, marginTop: 8 }}>
                ...
                {(resolutionForm.final_conclusion === 'conditional_pass' ||
                  ...
                  >
                    当前已有 {(data.remediation_issues || []).length}{' '}
                    个整改项，如不足可在上方整改项区域按需追加。
                  </div>
                )}
              </div>

              <button
                style={{ ...S.btn(true), marginTop: 12, background: '#faad14' }}
          ...
          .map((item: any) => {
            ...
            return (
              <div
                ...
                onClick={() => {
                  if (!isReadOnly) toggleCheck(item.template_id, nextStatus(s))
                }}
              >
          ...
          <div key={rn} style={{ marginBottom: 8 }}>
            ...
            >
              ...
              <span
                style={{
                  fontSize: 12,
                  color: pass === total ? '#52c41a' : '#faad14',
                  marginLeft: 'auto',
                }}
              >
                {pass}/{total}{' '}
              ...
              items
                ...
                .map((item: any) => {
                  ...
                  return (
                    ...
                    >
                      <span style={{ color: statusColor[s], fontSize: 14 }}>
                        {statusDisplay[s]}
                      </span>
