...
const STATUS_COLORS: Record<string, string> = {
  ...
  rejected: '#ff4d4f',
  ready: '#8c8c8c',
  awaiting_resolution: '#722ed1',
  resolution_published: '#13c2c2',
...
const S: Record<string, any> = {
  ...
  tab: (a: boolean): React.CSSProperties => ({
    ...
    cursor: 'pointer',
    fontSize: 14,
    borderBottom: a ? '2px solid #1677ff' : '2px solid transparent',
    color: a ? '#1677ff' : '#666',
    fontWeight: a ? 600 : 400,
  }),
  btn: (p: boolean, d = false): React.CSSProperties => ({
    padding: '6px 16px',
    border: p ? 'none' : '1px solid #d9d9d9',
    borderRadius: 4,
    background: d ? '#d9d9d9' : p ? '#1677ff' : '#fff',
    color: d ? '#999' : p ? '#fff' : '#333',
...
const App: React.FC = () => {
  ...
  const [passedPhasesByType, setPassedPhasesByType] = useState<Record<string, string[]>>({
    dcp: [],
    tr: [],
  })
  const [ipdFlowLayout, setIpdFlowLayout] = useState<any>(null)
  const [allReviews, setAllReviews] = useState<any[]>([])
  const [currentUser, setCurrentUser] = useState<{ uuid: string; name: string }>({
    uuid: '',
    name: '',
  })
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (ck) {
        ...
        if (tuid) {
          fetch(`/project/api/ones-project/team/${tuid}/projects/exchange/${ck[1]}`, {
            credentials: 'include',
          })
            ...
            .then((raw) => {
              ...
              if (realUuid) {
                ...
                loadList(realUuid)
                return fetch(
                  `/project/api/project/team/${tuid}/project/${realUuid}/stamps/data?t=project`,
                  {
                    method: 'POST',
                    credentials: 'include',
    ...
    if (!puid) {
      try {
        const m = document.referrer.match(/[?&]projectUUID=([^&]+)/)
        if (m) puid = m[1]
      } catch {}
    }
    if (puid) {
    ...
    if (tu) {
      fetch(`/project/api/project/team/${tu}/../../users/me`, { credentials: 'include' })
        ...
        .then((j) => {
          const d = j.data || j
          if (d.uuid) setCurrentUser({ uuid: d.uuid, name: d.name || '当前用户' })
        })
        .catch(() => {})
    }
    // 检查新建权限
  ...
  async function loadList(puid: string, rvType?: string) {
    ...
    try {
      ...
      setMsg(`加载评审列表失败: ${e.message}`)
    } finally {
      setLoading(false)
    }
  }

  // 新架构：评审单主体 = 系统自定义工作项。有工作项编号时直接跳原生工作项详情页
  // （评审过程在详情页「评审过程」Tab，评审动作在「IPD评审操作」快捷按钮）。
  function openReviewEntry(rv: any) {
    if (rv?.issue_number) {
      try {
        const url = `/project/#/team/${getTeamUUID()}/project/${rv.project_uuid || ''}/issue/${rv.issue_number}`
    ...
    loadDetail(rv?.review_uuid)
  }

  async function loadDetail(rid: string) {
    setLoading(true)
    try {
      const data = await api.getReviewDetail(rid)
      setDetail(data)
      setView('detail')
    } catch (e: any) {
      setMsg(`加载评审详情失败: ${e.message}`)
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleCreate(form: any) {
    ...
    try {
      ...
      if (Array.isArray(res.auto_reviewer_uuids) && res.auto_reviewer_uuids.length > 0) {
        try {
          await api.ensureProjectMembers(projectUuid, res.auto_reviewer_uuids)
        } catch (e: any) {
          memberSyncError = e?.message || '自动添加项目成员失败'
        }
      }
  ...
  async function handleRecreate(rid: string) {
    setMsg('')
    try {
      const res = (await api.recreateReview(rid, {
        operator_uuid: currentUser.uuid,
        project_uuid: projectUuid,
        project_identifier: projectKey,
      })) as any
      await loadList(projectUuid)
      await loadDetail(res.review_uuid)
    } catch (e: any) {
    ...
    return (
      <ReviewDetail
        ...
        onBack={() => {
          setView('list')
          setDetail(null)
          loadList(projectUuid)
        }}
        onRefresh={() => refreshDetail((detail.review as any).review_uuid)}
        onStart={handleStart}
  ...
  const filteredReviews = reviews.filter((r) => {
    if (
      filterNumber &&
      !(r.review_number || '').toLowerCase().includes(filterNumber.trim().toLowerCase())
    )
      return false
    if (filterPhase && r.phase_code !== filterPhase) return false
    if (
      filterTitle &&
      !(r.review_title || '').toLowerCase().includes(filterTitle.trim().toLowerCase())
    )
      return false
  ...
  return (
    <div style={S.container}>
      ...
      {view === 'list' && (
        <div
          style={{ display: 'flex', gap: 0, marginBottom: 16, borderBottom: '1px solid #e8e8e8' }}
        >
          {(['dcp', 'tr'] as const).map((t) => (
            <button
              ...
              style={{
                ...
                color: reviewType === t ? '#1677ff' : '#666',
                fontWeight: reviewType === t ? 600 : 400,
              }}
            >
              {t === 'dcp' ? 'DCP 评审' : 'TR 评审'} (
              {allReviews.filter((r: any) => (r.review_type || 'dcp') === t).length})
            </button>
          ))}
        </div>
      )}
      ...
      >
        <h3 style={{ fontSize: 16, margin: 0 }}>
          {reviewType === 'tr' ? '' : ''} {reviewType === 'tr' ? 'TR' : 'DCP'} 评审列表（
          {reviews.length}）
        </h3>
        <div>
          {hasCreatePerm && (
            <button style={S.btn(true)} onClick={() => setView('create')}>
              + 创建评审
            </button>
          )}
          <button style={S.btn(false)} onClick={() => loadList(projectUuid)}>
            刷新
          </button>
        </div>
      </div>
      ...
      {view === 'list' && reviewType === 'dcp' && phases.length > 0 && (
        <IPDFlowChart
          layout={ipdFlowLayout}
          phases={phases}
          reviews={allReviews}
          onPhaseClick={(payload: any) =>
            openReviewEntry(typeof payload === 'string' ? { review_uuid: payload } : payload)
          }
        />
      )}
      {view === 'create' && (
        <CreateReviewForm
          ...
          projectKey={projectKey}
          projectName={projectName}
          pkases={phases.filter((p: any) => (p.review_type || 'dcp') === reviewType)}
          passedPhases={passedPhasesByType[reviewType] || []}
      ...
      {reviews.length === 0 && view === 'list' ? (
        ...
        >
          <div style={{ fontSize: 14, marginBottom: 8 }}>
            该项目暂无 {reviewType === 'tr' ? 'TR' : 'DCP'} 评审单
          </div>
          <div style={{ fontSize: 12 }}>
            点击「创建评审」创建首个 {reviewType === 'tr' ? 'TR' : 'DCP'} 评审草稿
          </div>
        </div>
      ) : view === 'list' ? (
        <>
                        ...
                        >
                          ...
                          >
                            <span
                              ...
                              onClick={async (e) => {
                                e.stopPropagation()
                                const ok = await copyReviewLink(
                                  r.review_number || r.review_uuid,
                                  r.review_uuid,
                                )
                                setCopyToast(ok ? '已复制编号' : '复制失败')
                          ...
                          <td style={{ ...S.td, textAlign: 'center' }}>
                            {(() => {
                              const rsc =
                                RESOLUTION_COLORS[r.resolution_status] || r.final_conclusion || ''
                          ...
                          <td style={{ ...S.td, textAlign: 'center', fontSize: 12 }}>
                            {r.material_submitted || 0}/{r.material_total || 0}
                          </td>
                          <td style={{ ...S.td, textAlign: 'center', fontSize: 12 }}>
                            {r.reviewer_done}/{r.reviewer_total}
                          </td>
                          <td style={{ ...S.td, textAlign: 'center' }}>
                          ...
                          <td style={{ ...S.td, fontSize: 12, color: '#999' }}>
                            {r.meeting_time
                              ? new Date(r.meeting_time).toLocaleString('zh-CN')
                              : '-'}
                          </td>
                        </tr>
                      )
...
const UserPicker: React.FC<{
  ...
  const allowedSet = React.useMemo(
    () => (allowedUserIds && allowedUserIds.length > 0 ? new Set(allowedUserIds) : null),
    [allowedUserIds],
  )
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      />
      {loading && (
        <div style={{ position: 'absolute', right: 10, top: 6, fontSize: 12, color: '#999' }}>
          搜索中…
        </div>
      )}
    ...
    <div ref={ref} style={{ position: 'relative', width: '100%', maxWidth: 240 }}>
      <button
        ...
        style={{
          ...
          alignItems: 'center',
          justifyContent: 'space-between',
          boxShadow: open ? '0 0 0 2px rgba(22,119,255,0.12)' : 'none',
          textAlign: 'left',
        }}
      >
            ...
            return (
              <div
                ...
                style={{
                  ...
                  gap: 12,
                  fontSize: 13,
                  color: option.uuid ? '#262626' : '#8c8c8c',
                  background: selected ? '#e6f4ff' : '#fff',
                }}
                onMouseEnter={(event) => {
                  if (!selected) event.currentTarget.style.background = '#f5f5f5'
                }}
                onMouseLeave={(event) => {
              ...
              >
                ...
                {selected && (
                  <span aria-hidden="true" style={{ color: '#1677ff', fontWeight: 600 }}>
                    ✓
                  </span>
                )}
              </div>
  ...
  return (
    ...
    >
      ...
      {open && (
        ...
        >
          {profileName && (
            <div style={{ marginBottom: 6, color: '#262626', fontWeight: 600 }}>
              Profile：{profileName}
            </div>
          )}
  ...
  >
    ...
    >
      <div style={{ padding: '16px 20px', borderBottom: '1px solid #f0f0f0' }}>
        <h3 id="reviewer-message-title" style={{ margin: 0, fontSize: 16, color: '#262626' }}>
          {title}
        </h3>
      </div>
      ...
      <div style={{ padding: '12px 20px', borderTop: '1px solid #f0f0f0', textAlign: 'right' }}>
        <button type="button" style={{ ...S.btn(true), marginRight: 0 }} onClick={onClose}>
          我知道了
        </button>
      </div>
    </div>
...
const CreateReviewForm: React.FC<{
  ...
  const [title, setTitle] = useState(
    projectName && defaultPhaseName ? `${projectName} ${defaultPhaseName}` : '',
  )
  ...
  let deps: string[] = []
  try {
    deps = JSON.parse(selectedPhaseObj?.dependencies || '[]')
  } catch {
    deps = []
  }
  ...
  return (
    <div style={{ ...S.card, maxWidth: 480, margin: '0 auto' }}>
      <h3 style={{ fontSize: 16, margin: '0 0 16px 0' }}>
        创建 {reviewType === 'tr' ? 'TR' : 'DCP'} 评审草稿
      </h3>
      <div style={S.formGroup}>
        <label style={S.label}>所属项目</label>
        <input
          style={{ ...S.input, background: '#f5f5f5', color: '#666' }}
          value={projectName || projectKey || projectUuid || ''}
          disabled
          placeholder="当前项目"
        />
      </div>
      <div style={S.formGroup}>
        ...
        {pkases.length === 0 ? (
          <div style={{ color: '#999', fontSize: 12 }}>
            暂未配置阶段，请先在插件配置中设置 {reviewType === 'tr' ? 'TR' : 'DCP'} 阶段
          </div>
        ) : (
          <select
      ...
      {blocked && (
        ...
        >
          前置阶段未通过决议，创建草稿后仍无法正式发起评审。需先通过（决议为"通过"或"有条件通过"）：
          <strong>{unmetDeps.join(', ')}</strong>
        </div>
      )}
      <div style={S.formGroup}>
        ...
        <button
          ...
          onClick={() =>
            onCreate({
              phase_code: phase,
              review_title: title,
              meeting_time: meetingTime ? new Date(meetingTime).getTime() : 0,
              creator_uuid: currentUser.uuid || '',
              review_type: reviewType,
            })
          }
          disabled={pkases.length === 0 || !phase}
        >
          创建评审草稿
        </button>
...
export const ReviewDetail: React.FC<{
  ...
  onBack: () => void
  onRefresh: () => void
  onStart: (rid: string) => void
  onRecreate: (rid: string) => void
  msg: string
  ...
  const TABS: { key: TabKey; label: string; badge?: number }[] = [
    {
      key: 'materials',
      label: '评审资料',
      badge: data.materials?.filter((m: any) => !!m.file_data).length,
    },
    {
      key: 'reviewers',
      label: '评审人与意见',
      badge: data.reviewers?.filter((r: any) => r.submitted_at > 0).length,
    },
  ...
  const [linkRemediationForm, setLinkRemediationForm] = useState({
    issue_uuid: '',
    issue_number: '',
    issue_title: '',
  })
  ...
  useEffect(() => {
    fetch('/project/api/project/users/me', { credentials: 'include' })
      .then((r) => (r.ok ? r.json() : null))
      .then((me) => {
        if (me) setCurrentUser({ uuid: me.uuid || '', name: me.name || '' })
      })
      .catch(() => {})
  }, [])
  ...
  useEffect(() => {
    api
      ...
      .then((c) => {
        ...
        setConfigRoles(
          (c.roles || []).filter((r: any) => (r.review_type || 'dcp') === rvReviewType),
        )
      ...
      .catch(() => {})
      .finally(() => setRemediationConfigLoaded(true))
  }, [rvReviewType])
  ...
  useEffect(() => {
    ...
    resolveProjectIssueTypes(getTeamUUID(), projectUuid)
      .then((result) => {
        if (canceled) return
  ...
  const remediationTypeStatus: 'loading' | 'available' | 'missing' | 'unknown' | 'unconfigured' =
    !remediationConfigLoaded || projectTypesLoading
      ? 'loading'
      : !remediationIssueType && !remediationIssueTypeUuid
        ? 'unconfigured'
        : !projectTypesVerified
          ? 'unknown'
          : configuredRemediationType
            ? 'available'
            : 'missing'
  ...
  // 判断当前用户是否可以撤回评审
  const canRecall =
    recallConfig.enabled && isReviewing && !data.resolution && currentUser.uuid === rv.creator_uuid
  ...
  async function handleRecall() {
    ...
    setRecalling(true)
    try {
      const res = (await api.recallReview(rv.review_uuid, {
        operator_uuid: currentUser.uuid,
        reason: recallReason,
      })) as any
      if (res?.error) {
  ...
  const canEditMeetingTime =
    currentUser.uuid === rv.creator_uuid &&
    (rv.status === 'draft' || rv.status === 'reviewing') &&
    !data.resolution
  ...
  async function handleDeleteReview() {
    ...
    if (
      !confirm(
        `确定删除「${rv.phase_name || rv.phase_code} — ${rv.review_title || 'DCP评审'}」？此操作不可恢复。`,
      )
    )
      return
    try {
  ...
  async function handleRecreate() {
    ...
    const hint = hasFiles ? '已沿用上次评审的材料附件，请确认是否需要更新。' : ''
  ...
  async function syncRemediationFromBrowser(): Promise<boolean> {
    const remediationIssues = (data.remediation_issues || data.linked_issues || []).filter(
      (iss: any) => iss.link_type === 'remediation',
    )
  ...
  async function handleRefreshRemediation() {
    ...
    try {
      ...
      setRemediationMsg(e.message || '刷新失败')
    } finally {
      setRemediationRefreshing(false)
    }
  }
  ...
  async function handleConfirmRemediation(nextAction: 're_review') {
    ...
    try {
      ...
      setRemediationMsg(e.message || '操作失败')
    } finally {
      setRemediationConfirming(false)
    }
  }

  async function handleCreateRemediationIssue() {
    if (!createRemediationForm.title.trim()) {
      setRemediationMsg('请填写工作项标题')
      return
    }
    ...
    try {
      ...
      if (!lookup.verified)
        throw new Error('无法确认当前项目的工作项类型，不允许新建整改项。请刷新后重试。')
      const configured = findConfiguredIssueType(
        lookup.types,
        remediationIssueType,
        remediationIssueTypeUuid,
      )
      ...
      if (add3Res.ok) {
        ...
        if (task?.uuid) {
          try {
            ...
            setRemediationMsg(
              `工作项 ${task.display_id || task.uuid} 已创建，但关联评审单失败：${linkError.message || '未知错误'}`,
            )
            onRefresh()
            return
          }
  ...
  async function handleLinkRemediationIssue() {
    if (!linkRemediationForm.issue_uuid.trim()) {
      setRemediationMsg('请填写工作项 UUID')
      return
    }
    ...
    try {
      ...
      setRemediationMsg(e.message || '关联失败')
    } finally {
      setLinkingRemediation(false)
    }
  }
  ...
  async function handleRemind(target: 'reviewers' | 'resolution') {
    ...
    setMsg('')
    try {
      const res = (await api.remindReview(rv.review_uuid, {
        target,
        operator_uuid: currentUser.uuid,
    ...
    <div style={S.container}>
        ...
        <div style={S.topRow}>
          <div>
            <span
              ...
              title="点击复制编号"
              onClick={async () => {
                const ok = await copyReviewLink(rv.review_number || rv.review_uuid, rv.review_uuid)
                setCopyToast(ok ? '已复制编号' : '复制失败')
                setTimeout(() => setCopyToast(''), 2000)
              }}
            >
              {(rv.review_type || 'dcp') === 'tr' ? '' : ''} {rv.review_number || ''}
            </span>
            <strong style={{ fontSize: 16 }}>
              {rv.phase_name || rv.phase_code} —{' '}
              {rv.review_title || ((rv.review_type || 'dcp') === 'tr' ? 'TR评审' : 'DCP评审')}
            </strong>
            <span style={S.statusTag(sc)}>{sl}</span>
            {data.resolution && (
              <span style={{ marginLeft: 8, fontSize: 12, color: '#666' }}>
                快照: {data.resolution.snapshot_number}
              </span>
            )}
          </div>
          <div>
            {isEditable && (
              <button style={S.btn(true)} onClick={() => onStart(rv.review_uuid)}>
                ▶ 发起评审
              </button>
            )}
            {isEditable && currentUser.uuid && rv.creator_uuid === currentUser.uuid && (
              <button
                style={{ ...S.btn(false), color: '#ff4d4f', borderColor: '#ff4d4f' }}
                onClick={handleDeleteReview}
              >
                删除
              </button>
            )}
            {isReviewing && !data.resolution && canPublish && (
              <button
                style={S.btn(true)}
                onClick={() => {
              ...
              >
                {showPublishForm ? '× 取消发布' : '生成决议'}
              </button>
            )}
            {canRemindReviewers && (
              <button
                style={{ ...S.btn(false), color: '#faad14', borderColor: '#faad14' }}
                onClick={() => handleRemind('reviewers')}
                disabled={reminding}
              >
                {reminding ? '催办中…' : `催办评审人(${pendingReviewers.length})`}
              </button>
            )}
            {canRemindResolution && (
              <button
                style={{ ...S.btn(false), color: '#faad14', borderColor: '#faad14' }}
                onClick={() => handleRemind('resolution')}
                disabled={reminding}
              >
                {reminding ? '催办中…' : '催办决议人'}
              </button>
            )}
            {canRecall && !showRecallForm && (
              <button
                style={{ ...S.btn(false), color: '#faad14', borderColor: '#faad14' }}
                onClick={() => {
            ...
            {rv.status === 'rejected' &&
              currentUser.uuid &&
              rv.creator_uuid === currentUser.uuid && (
                <button
                  style={{ ...S.btn(true), background: '#722ed1', borderColor: '#722ed1' }}
            ...
            {effState === 'remediation_pending' &&
              ...
              !showReReviewConfirm && (
                <button
                  style={{ ...S.btn(true), background: '#fa8c16', borderColor: '#fa8c16' }}
                  onClick={() => {
          ...
          {editingMeetingTime ? (
            <span style={{ marginLeft: 12, display: 'inline-flex', alignItems: 'center', gap: 4 }}>
              ...
              <button
                ...
                disabled={savingMeetingTime}
              >
                {savingMeetingTime ? '保存中…' : '保存'}
              </button>
              <button
                style={{
                  padding: '2px 8px',
            ...
            <span style={{ marginLeft: 12 }}>
              会议:{' '}
              {rv.meeting_time > 0 ? new Date(rv.meeting_time).toLocaleString('zh-CN') : '未设置'}
              {canEditMeetingTime && (
                <button
                  style={{
                    marginLeft: 4,
        ...
        (() => {
          ...
          return (
            ...
            >
              第 {_rNo} 轮复审
              {_prevRes
                ? `（上一轮决议：${_prevRes.final_conclusion === 'conditional_pass' ? '有条件通过' : _prevRes.final_conclusion === 'rework' ? '返工' : _prevRes.final_conclusion}）`
                : ''}
              。整改项已完成，评审人需重新提交评审意见，决议人需重新发布决议。
            </div>
          )
        })()}
      {/* 撤回评审表单 */}
      {showRecallForm && (
        ...
        >
          ...
          <div style={{ fontSize: 13, color: '#666', marginBottom: 12 }}>
            撤回后评审单将回到草稿状态，已提交的评审意见和 Checklist
            将被清空，评审人待办和决议待办将失效。
          </div>
      ...
      {showPublishForm && (
        ...
        >
          ...
          <div style={{ marginBottom: 8, fontSize: 13, color: '#333' }}>
            {(() => {
              ...
              if (submitMode === 'must_vote_roles') {
                const mustVoteNames = configRoles
                  .filter((r: any) => r.must_vote)
          ...
          {(() => {
            ...
            if (!rule) return null
            const approvalConclusions = rule.passRule?.approvalConclusions || [
              'pass',
              'conditional_pass',
            ]
            ...
            if (passMode === 'min_approval_count') {
              ...
              if (vsMode === 'all_reviewers') {
                ...
                scopeNames = configRoles
                  .filter((r: any) => selectedRoles.includes(r.role_name))
                  .map((r: any) => r.role_name)
              } else {
                scopeNames = configRoles
                  .filter((r: any) => r.must_vote)
              ...
              const acceptCount = candidates.filter((r: any) =>
              ...
              if (totalCandidates < minCount) {
                return (
                  ...
                  >
                    可计票评审人仅 ${totalCandidates} 人，规则要求至少 ${minCount}{' '}
                    人通过，请补充评审人或调整规则
                  </div>
                )
              }
              ...
              return (
                <div
                  style={{
                    ...
                    borderRadius: 4,
                    fontSize: 13,
                    background: canPass ? '#f6ffed' : '#fff7e6',
                    color: canPass ? '#52c41a' : '#faad14',
                  }}
                >
                  {canPass
              ...
              return (
                <div
                  style={{
                    ...
                    borderRadius: 4,
                    fontSize: 13,
                    background: allMustDone ? '#f6ffed' : '#fff7e6',
                    color: allMustDone ? '#52c41a' : '#faad14',
                  }}
                >
                  {allMustDone
            ...
            return (
              <div
                style={{
                  ...
                  borderRadius: 4,
                  fontSize: 13,
                  background: allDone ? '#f6ffed' : '#fff7e6',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <div style={S.formGroup}>
              ...
              <select
                style={{ ...S.select, width: '100%' }}
                value={resolutionForm.final_conclusion}
                onChange={(e) =>
                  setResolutionForm({ ...resolutionForm, final_conclusion: e.target.value })
                }
              >
                <option value="" disabled>
                  请选择结论
                </option>
                {(resolutionRule?.allowedConclusions || ['pass', 'conditional_pass', 'reject']).map(
                  (c: string) => {
                    const labels: any = {
                      pass: '✅ 通过',
                      conditional_pass: '⚠️ 有条件通过',
          ...
          <div style={{ ...S.formGroup, marginTop: 8 }}>
            ...
            <textarea
              style={S.textarea}
              rows={3}
              value={resolutionForm.condition_notes}
              onChange={(e) =>
                setResolutionForm({ ...resolutionForm, condition_notes: e.target.value })
              }
              placeholder="（可选）如为'有条件通过'，请说明条件…"
            />
          </div>
          {/* 评审意见预览 */}
                ...
                <tbody>
                  {(data.reviewers || []).map((r: any, i: number) => (
                    <tr key={i}>
                      ...
                      >
                        {r.opinion_summary || (r.submitted_at > 0 ? '(无文字)' : '—')}
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          {(resolutionForm.final_conclusion === 'conditional_pass' ||
            resolutionForm.final_conclusion === 'rework') && (
            <div
              style={{
                marginTop: 8,
                padding: '8px 12px',
            ...
            >
              <strong>整改要求</strong>：发布「有条件通过」或「返工」决议前，必须先在「整改项」tab
              中创建至少 1 个整改工作项。
              {(data.remediation_issues || []).length > 0 ? (
                <span style={{ color: '#52c41a' }}>
                  {' '}
                  已创建 {data.remediation_issues.length} 个整改项。
                </span>
              ) : (
          ...
          {(() => {
            ...
            const unchecked = (data.checklist || []).filter(
            ...
            return (
              ...
              >
                ...
                {indBlock && (
                  <div>
                    · 指标红线 {redInds.length} 项：
                    {redInds
                      .map((i: any) => i.indicator_name || '')
                      .filter(Boolean)
                      .join('、')}
                  </div>
                )}
          ...
          {gateViolations.length > 0 && (
            ...
            >
              ...
              {gateViolations.map((v: any, i: number) => (
                <div key={i}>
                  ·{' '}
                  {v.type === 'indicator_red'
                    ? `指标红线（${(v.items || []).length} 项）`
                    : v.type === 'checklist_incomplete'
                      ? `Checklist 未全勾（${(v.items || []).length} 项）`
                      : v.type === 'prerequisite_not_passed'
                        ? `前置阶段未通过（${(v.items || []).map((x: any) => x.phase_code).join('、')}）`
                        : v.type}
                </div>
              ))}
              <button
                style={{ ...S.btn(false), marginTop: 6, borderColor: '#faad14', color: '#fa8c16' }}
                onClick={() => {
                  setResolutionForm({
          ...
          <div style={{ marginTop: 12 }}>
            <button
              style={{ ...S.btn(true), background: '#faad14' }}
      ...
      {activeTab === 'materials' && (
        <div style={S.tabPanel}>
          <MaterialsPanel
            data={data}
            editable={data.can_edit_evidence === true}
      ...
      {activeTab === 'resolution' && (
        <div style={S.tabPanel}>
          <ResolutionPanel data={data} onRefresh={onRefresh} />
        </div>
      )}
      {activeTab === 'audit' && (
        <div style={S.tabPanel}>
          <AuditPanel reviewUuid={rv.review_uuid} />
        </div>
      )}
      {activeTab === 'timeline' && (
        <div style={S.tabPanel}>
          <StateTimeline data={data} effState={effState} roundNo={rv.round_no || 1} />
        </div>
      )}
      {activeTab === 'compare' && (
        <div style={S.tabPanel}>
...
const MaterialsPanel: React.FC<{
  ...
  onRefresh: () => void
  currentUser: { uuid: string; name: string }
}> = ({ data, editable, isRemediation, onRefresh, currentUser }) => {
  ...
  function downloadMaterial(templateId: string) {
    api
      .getMaterialDownloadUrl(data.review.review_uuid, templateId)
      .then((r: any) => {
        if (!r.url) return
  ...
  function downloadAttachment(objectKey: string) {
    api
      .getAttachmentDownloadUrl(data.review.review_uuid, objectKey)
      .then((r: any) => {
        if (!r.url) return
        const a = document.createElement('a')
        a.href = r.url
  ...
  async function changeMatStatus(templateId: string, status: string) {
    try {
      await api.updateMaterialStatus(data.review.review_uuid, {
  ...
  function handleFileSelect(templateId: string, file: File) {
    ...
    api
      .getMaterialUploadUrl(data.review.review_uuid, templateId)
      .then((presign: any) => {
  ...
  function startEditInd() {
    ...
    inds.forEach((i: any) => {
      v[i.template_id] = String(i.current_value ?? '')
      n[i.template_id] = i.notes || ''
    })
    setIndValues(v)
    setIndNotes(n)
    setEditInd(true)
  ...
  async function saveInd() {
    const payload = Object.entries(indValues)
      .filter(([, v]) => v.trim() !== '')
      .map(([tid, v]) => ({
        template_id: tid,
        current_value: parseFloat(v) || 0,
        notes: indNotes[tid] || '',
      }))
    try {
      await api.updateIndicators(data.review.review_uuid, { indicators: payload })
      setEditInd(false)
      onRefresh()
    } catch (e: any) {
      console.error(e)
    ...
    <div>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        >
          <h4 style={S.sectionTitle}>
            交付物清单（已上传 {mats.filter((m: any) => !!m.file_data).length}/{mats.length}）
          </h4>
        ...
        <div style={S.tableWrap}>
          <table style={S.table}>
            ...
            <tbody>
              {mats.length === 0 ? (
                <tr>
                  <td colSpan={2} style={{ ...S.td, textAlign: 'center', color: '#999' }}>
                    无交付物
                  </td>
                </tr>
              ) : (
                  ...
                  return (
                    <tr key={i}>
                      <td style={S.td}>
                        ...
                        {m.template?.material_name || m.template_id}
                        {isRemediated && (
                          <span
                            style={{
                              fontSize: 10,
                      ...
                      <td style={S.td}>
                          ...
                          >
                            ...
                            {hasFile && (
                              <span
                                ...
                                title={`点击预览: ${m.file_name}`}
                              >
                                <span
                                  style={{ cursor: 'pointer', textDecoration: 'underline' }}
                                  onClick={() => previewMaterial(m.template_id, m.file_name)}
                                >
                                  {m.file_name}
                                </span>
                                <button
                                  ...
                                  onClick={() => downloadMaterial(m.template_id)}
                                >
                                  下载
                                </button>
                                <button
                                  ...
                                  onClick={async () => {
                                    ...
                                    try {
                                      await api.removeMaterialFile(data.review.review_uuid, {
                                ...
                                {attachments.map((att: any, ai: number) => (
                                  ...
                                  >
                                    ...
                                    <span
                                      style={{ cursor: 'pointer', textDecoration: 'underline' }}
                                      onClick={() =>
                                        previewAttachment(att.file_data, att.file_name)
                                      }
                                    >
                                      {att.file_name}
                                    </span>
                                    <button
                                      ...
                                      onClick={() => downloadAttachment(att.file_data)}
                                    >
                                      下载
                                    </button>
                                  </div>
                        ...
                        {uploadMsg[m.template_id] && (
                          <div
                            style={{
                              fontSize: 11,
                              marginTop: 2,
                              color: uploadMsg[m.template_id].startsWith('上传失败')
                                ? '#ff4d4f'
                                : '#1677ff',
                            }}
                          >
      ...
      <div>
        ...
        >
          ...
          {editable && !editInd && (
            <button style={S.btn(false)} onClick={startEditInd}>
              编辑
            </button>
          )}
        </div>
          ...
          <table style={S.table}>
            <thead>
              <tr>
                <th style={S.th}>指标</th>
                <th style={S.th}>单位</th>
                <th style={{ ...S.th, width: 60, textAlign: 'center' }}>黄</th>
                <th style={{ ...S.th, width: 60, textAlign: 'center' }}>红</th>
                <th style={{ ...S.th, width: 90, textAlign: 'center' }}>当前值</th>
                <th style={{ ...S.th, width: 60, textAlign: 'center' }}>风险</th>
              </tr>
            </thead>
            <tbody>
              {inds.length === 0 ? (
                      ...
                      <td style={{ ...S.td, textAlign: 'center' }}>
                        {editInd ? (
                          <input
                            ...
                            value={indValues[ind.template_id] || ''}
                            onChange={(e) =>
                              setIndValues({ ...indValues, [ind.template_id]: e.target.value })
                            }
                          />
                        ) : (
                          <span style={{ fontWeight: 600 }}>
                            {ind.current_value} {tpl?.unit}
                          </span>
                        )}
                      </td>
        ...
        {editInd && (
          <div style={{ marginTop: 12, display: 'flex', gap: 8 }}>
            <button style={S.btn(true)} onClick={saveInd}>
              保存
            </button>
            <button style={S.btn(false)} onClick={() => setEditInd(false)}>
              取消
            </button>
          </div>
        )}
...
const ReviewersPanel: React.FC<{
  ...
  onRefresh: () => void
  currentUser: { uuid: string; name: string }
}> = ({ data, projectUuid, projectName, editable, isReviewing, onRefresh, currentUser }) => {
  ...
  const profileSnapshot = React.useMemo(() => {
    try {
      ...
      const map: Record<
        string,
        { mode: 'single' | 'pool'; default_reviewer_uuid: string; candidate_uuids: string[] }
      > = {}
      for (const item of arr) {
        ...
        map[item.role_name] = {
          ...
          candidate_uuids: Array.isArray(item.candidate_uuids)
            ? item.candidate_uuids.filter((u: any) => !!u).map(String)
            : [],
        }
      }
  ...
  function getRoleRestriction(roleName: string): {
  ...
  useEffect(() => {
    if (editable && roles.length > 0) {
      ...
      reviewers.forEach((r: any) => {
        sel[r.role_name] = r.reviewer_uuid || ''
      })
  ...
  async function saveReviewers() {
    ...
    if (missingRequired.length > 0) {
      setMessageDialog({
        title: '请完善评审人',
        message: `以下角色为必选：${missingRequired.join('、')}`,
      })
      return
    }
    ...
    if (profileSnapshot) {
      for (const role of roles) {
        ...
        if (
          snap.mode === 'single' &&
          snap.default_reviewer_uuid &&
          currentUuid &&
          currentUuid !== snap.default_reviewer_uuid
        ) {
          setMessageDialog({
            title: '无法更换默认评审人',
            message: `角色「${role.role_name}」已由 Profile 锁定默认人选。`,
          })
          return
        }
      }
    ...
    const list = Object.entries(selected)
      .filter(([, uid]) => uid.trim())
      .map(([role, uid]) => ({ role_name: role, reviewer_uuid: uid }))
    setSavingReviewers(true)
    try {
      await api.ensureProjectMembers(
        projectUuid,
        list.map((item) => item.reviewer_uuid),
      )
      ...
      if (errorData.code === 'PROJECT_MEMBER_ADD_FAILED') {
        ...
        setMessageDialog({
          title: '保存评审人失败',
          message: e?.message || errorData.error || '保存失败',
        })
      }
      onRefresh()
    } finally {
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        >
          <h4 style={S.sectionTitle}>
            评审人（{reviewers.length}人，已提交{' '}
            {reviewers.filter((r: any) => r.submitted_at > 0).length}）
          ...
          <div>
            {roles.length === 0 ? (
              ...
              <div>
                {!publisherRole && (
                  ...
                  >
                    ⚠ 当前{reviewType.toUpperCase()}
                    未配置决议角色，请先在「决议规则」中选择一个决议角色。
                  </div>
                )}
                <div style={S.tableWrap}>
                  <table style={S.table}>
                    <thead>
                      <tr>
                        <th style={S.th}>角色</th>
                        <th style={{ ...S.th, width: 160 }}>角色属性</th>
                        <th style={S.th}>
                          评审人
                          <ReviewerHelp profileName={data.review?.reviewer_profile_name} />
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                        ...
                        return (
                          <tr key={role.role_name}>
                            ...
                            <td style={{ ...S.td, fontSize: 11 }}>
                              ...
                              {!isPublisher && !role.must_vote && !role.has_veto && (
                              ...
                              {(() => {
                                ...
                                return (
                                  <>
                                    ...
                                    {isPublisher && !selected[role.role_name] && (
                                      <div style={{ fontSize: 11, color: '#ff4d4f', marginTop: 2 }}>
                                        决议角色必须指定 1 名人员
                                      </div>
                                    )}
                                    {restriction.allowedUserIds &&
                                      restriction.allowedUserIds.length === 0 && (
                                        <div
                                          style={{ fontSize: 11, color: '#ff4d4f', marginTop: 2 }}
                                        >
                                          此角色的 Profile 设置了单人模式但未指定默认评审人
                                        </div>
                                      )}
                                  </>
                                )
            ...
            <div style={{ marginTop: 12, display: 'flex', gap: 8, alignItems: 'center' }}>
              <button
                ...
                disabled={savingReviewers || roles.length === 0}
              >
                {savingReviewers ? '保存中…' : '保存评审人'}
              </button>
              {reviewerDirty && (
                <span style={{ color: '#faad14', fontSize: 12 }}>评审人配置有未保存修改</span>
              )}
            </div>
          </div>
        ) : (
                ...
                <table style={S.table}>
                  <thead>
                    <tr>
                      ...
                      <th style={{ ...S.th, width: 100, textAlign: 'center' }}>结论</th>
                  ...
                  <tbody>
                    {reviewers.map((r: any, i: number) => (
                      <tr key={i}>
                        ...
                        <td style={S.td}>
                          {r.reviewer_uuid ? nameMap[r.reviewer_uuid] || '未知成员' : '-'}
                        </td>
                        <td style={{ ...S.td, textAlign: 'center' }}>
                          {r.conclusion ? CONCLUSION_LABELS[r.conclusion] || r.conclusion : '-'}
                        </td>
                        <td style={{ ...S.td, textAlign: 'center' }}>
                          {r.risk_level === 'low'
                            ? '低'
                        ...
                        <td style={{ ...S.td, textAlign: 'center' }}>
                          <span
                            style={{
                              ...
                              borderRadius: 4,
                              fontSize: 11,
                              background: r.submitted_at > 0 ? '#f6ffed' : '#fafafa',
                              color: r.submitted_at > 0 ? '#52c41a' : '#999',
                            }}
                          >
...
// ============================================================
const LinkedIssuesPanel: React.FC<{
  data: any
  projectUuid: string
  projectKey: string
  componentUuid: string
  viewUuid: string
  onRefresh: () => void
}> = ({ data, projectUuid, projectKey, componentUuid, viewUuid, onRefresh }) => {
  ...
  for (const iss of issues) {
    ...
    if (!byCreator[key])
      byCreator[key] = { name: iss.linked_by_name || key.substring(0, 8), count: 0 }
    byCreator[key].count++
  }
  ...
  return (
    <div>
      ...
      {issues.length === 0 ? (
        <div style={{ color: '#999', padding: 20, textAlign: 'center' }}>暂无关联工作项</div>
      ) : (
        <div style={S.tableWrap}>
          <table style={S.table}>
            <thead>
              <tr>
                <th style={S.th}>ID</th>
                <th style={S.th}>标题</th>
                <th style={{ ...S.th, width: 80 }}>工作项类型</th>
                <th style={{ ...S.th, width: 80 }}>状态</th>
              ...
              {issues.map((iss: any, i: number) => (
                <tr key={i}>
                  <td style={S.td}>
                    ...
                    >
                      {iss.issue_number || iss.issue_uuid?.substring(0, 12)}
                    </a>
                  </td>
                  <td style={S.td}>
                    <a
                  ...
                  <td style={S.td}>
                    {iss.linked_by_name || (iss.linked_by ? iss.linked_by.substring(0, 8) : '-')}
                  </td>
                  <td style={{ ...S.td, fontSize: 12, color: '#999' }}>
                    {iss.linked_at ? new Date(iss.linked_at).toLocaleString('zh-CN') : '-'}
                  </td>
                </tr>
              ))}
            </tbody>
...
function SnapshotSections({
  ...
  const raw = res.based_on_votes
    ? (() => {
        try {
          return JSON.parse(res.based_on_votes)
        } catch {
          return null
        }
      })()
    ...
    <div style={{ marginTop: 12 }}>
      <div style={sectionBtn('', 0)} onClick={() => setExpanded(!expanded)}>
        ...
        <span style={{ fontSize: 11, color: '#999', fontWeight: 400 }}>
          （评审意见 {votes.length} · 指标 {indicators.length} · Checklist {checklist.length} ·
          整改项 {issues.length}）
        </span>
      </div>
                        ...
                        <span
                          style={{
                            color:
                              v.conclusion === 'pass'
                                ? '#52c41a'
                                : v.conclusion === 'conditional_pass'
                                  ? '#faad14'
                                  : '#ff4d4f',
                          }}
                        >
                          {CONCLUSION_LABELS[v.conclusion] ||
                            (v.submitted_at > 0 ? v.conclusion : '—')}
                        </span>
                      </td>
                      <td style={{ ...S.td, textAlign: 'center' }}>
                        {RISK_LABELS[v.risk_level] || '中'}
                      </td>
                      <td
                        style={{
                          ...S.td,
              ...
              <table style={S.table}>
                <thead>
                  <tr>
                    ...
                    <th style={{ ...S.th, width: 80, textAlign: 'center' }}>当前值</th>
                    <th style={{ ...S.th, width: 60, textAlign: 'center' }}>风险</th>
                    <th style={S.th}>备注</th>
                  </tr>
                </thead>
          ...
          {checklist.length > 0 && (
            <div style={{ marginBottom: 12 }}>
              ...
              <table style={S.table}>
                <thead>
                  <tr>
                    <th style={{ ...S.th, width: 50, textAlign: 'center' }}>状态</th>
                    <th style={S.th}>检查项</th>
                    <th style={{ ...S.th, width: 80 }}>检查人</th>
                    <th style={{ ...S.th, width: 120 }}>检查时间</th>
                  </tr>
                </thead>
                <tbody>
                  {checklist.map((item: any, i: number) => (
                    <tr key={i}>
                      ...
                      <td style={S.td}>
                        {item.checked_at ? new Date(item.checked_at).toLocaleString('zh-CN') : '-'}
                      </td>
                    </tr>
                  ))}
                </tbody>
                ...
                <tbody>
                  {issues.map((iss: any, i: number) => (
                    <tr key={i}>
                      <td style={S.td}>
                        <a
                          href={`/project/#/team/${getTeamUUID()}/project/${projectUuid || ''}/issue/${iss.issue_number || iss.issue_uuid}`}
                          target="_blank"
                          style={{
                            color: '#1677ff',
                            textDecoration: 'none',
                        ...
                        >
                          {iss.issue_number || iss.issue_uuid?.substring(0, 8)}
                        </a>
                      </td>
...
const ResolutionPanel: React.FC<{ data: any; onRefresh: () => void }> = ({ data, onRefresh }) => {
  ...
  const prevResolutions = allResolutions
    .filter((r: any) => (r.round_no || 1) < _rNo)
          ...
          {prevResolutions.map((pr: any, i: number) => (
            ...
            >
              <div style={{ fontSize: 12, color: '#999', marginBottom: 4 }}>
                第{pr.round_no || 1}轮 | 快照: <code>{pr.snapshot_number}</code> |{' '}
                {pr.published_at ? new Date(pr.published_at).toLocaleString('zh-CN') : '-'}
              </div>
              <div style={{ fontSize: 14, fontWeight: 600 }}>
                ...
                <span
                  style={{
                    color:
                      pr.final_conclusion === 'pass'
                        ? '#52c41a'
                        : pr.final_conclusion === 'conditional_pass'
                          ? '#faad14'
                          : '#ff4d4f',
                  }}
                >
              ...
              {pr.condition_notes && (
                <div style={{ whiteSpace: 'pre-wrap', fontSize: 12, color: '#666', marginTop: 4 }}>
                  {pr.condition_notes}
                </div>
              )}
              {pr.published_by_name && (
                <div style={{ fontSize: 12, color: '#999', marginTop: 4 }}>
                  发布人: {pr.published_by_name}
                </div>
              )}
    ...
    <div>
      ...
      <div style={{ ...S.card, borderLeft: '4px solid #1677ff', background: '#f0f5ff' }}>
        ...
        <div style={{ fontSize: 12, color: '#666', marginBottom: 8 }}>
          快照编号: <code>{res.snapshot_number}</code> | 发布时间:{' '}
          {res.published_at ? new Date(res.published_at).toLocaleString('zh-CN') : '-'}
        </div>
        <div style={{ fontSize: 16, fontWeight: 600, marginBottom: 8 }}>
          ...
          <span
            style={{
              color:
                res.final_conclusion === 'pass'
                  ? '#52c41a'
                  : res.final_conclusion === 'conditional_pass'
                    ? '#faad14'
                    : '#ff4d4f',
            }}
          >
            {res.final_conclusion === 'pass'
              ...
              : res.final_conclusion === 'conditional_pass'
                ? '⚠️ 有条件通过'
                : res.final_conclusion === 'fail'
                  ? '❌ 不通过'
                  : res.final_conclusion === 'rework'
                    ? '🔧 返工'
                    : '❌ 驳回'}
          </span>
        </div>
        ...
        {res.published_by_name && (
          <div style={{ marginTop: 8, fontSize: 12, color: '#666' }}>
            发布人: {res.published_by_name}
          </div>
        )}
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          {supps.map((s: any, i: number) => (
            <div
              ...
              style={{
                ...S.card,
                borderLeft: `3px solid ${s.note_type === 'rectification' ? '#faad14' : '#1677ff'}`,
              }}
            >
              ...
              <div style={{ whiteSpace: 'pre-wrap', fontSize: 13, color: '#666' }}>
                {s.note_content}
              </div>
            </div>
          ))}
...
const ACTION_LABELS: Record<string, string> = {
  ...
  remove_material: '删除材料',
  update_reviewers: '更新评审人',
  submit_opinion: '提交评审意见',
  link_issue: '关联工作项',
  create_issue: '创建工作项',
  publish_resolution: '发布决议',
  add_supplement: '添加补充说明',
  add_correction: '添加纠偏说明',
}
...
const AuditPanel: React.FC<{ reviewUuid: string }> = ({ reviewUuid }) => {
  ...
  useEffect(() => {
    api
      ...
      .then((data) => {
        ...
        if (uuids.length > 0) {
          api
            .resolveReviewerNames(uuids as string[])
            .then(setNameMap)
            .catch(() => {})
        }
      })
  ...
  if (loading)
    return <div style={{ textAlign: 'center', color: '#999', padding: 20 }}>加载审计日志…</div>

  return (
    <div>
      <h4 style={S.sectionTitle}>审计日志（{logs.length}条）</h4>
      {logs.length === 0 ? (
        <div style={{ color: '#999', padding: 20, textAlign: 'center' }}>暂无审计记录</div>
      ) : (
        <div style={S.tableWrap}>
          <table style={{ ...S.table, marginBottom: 0 }}>
            <thead>
              <tr>
                <th style={{ ...S.th, width: 140 }}>时间</th>
                <th style={{ ...S.th, width: 100 }}>操作者</th>
                <th style={{ ...S.th, width: 120 }}>动作</th>
                <th style={S.th}>详情</th>
            ...
            <tbody>
              {logs.map((l: any, i: number) => (
                <tr key={i}>
                  ...
                  <td style={{ ...S.td, fontSize: 11 }}>
                    {l.operator_uuid ? nameMap[l.operator_uuid] || '未知用户' : '-'}
                  </td>
                  ...
                  <td style={{ ...S.td, textAlign: 'center' }}>
                    <span
                      style={{
                        color: l.result === 'success' ? '#52c41a' : '#ff4d4f',
                        fontSize: 12,
                      }}
                    >
                      {l.result}
...
const ReviewerWorkspace: React.FC<{
  ...
  onRefresh: () => void
  setMsg: (v: string) => void
}> = ({ data, projectUuid, onRefresh, setMsg }) => {
  const mats = data.materials || []
  const inds = data.indicators || []
  const rv = data.review
  ...
  function downloadMaterial(templateId: string) {
    api
      .getMaterialDownloadUrl(rv.review_uuid, templateId)
      .then((r: any) => {
        if (!r.url) return
        const a = document.createElement('a')
        a.href = r.url
  ...
  function downloadAttachment(objectKey: string) {
    api
      .getAttachmentDownloadUrl(rv.review_uuid, objectKey)
      .then((r: any) => {
        if (!r.url) return
        const a = document.createElement('a')
        a.href = r.url
  ...
  return (
    <div>
      ...
      <div style={{ ...S.card, marginBottom: 16 }}>
        <h4 style={S.sectionTitle}>
          评审资料（已上传 {mats.filter((m: any) => !!m.file_data).length}/{mats.length}）
        </h4>
        <div style={S.tableWrap}>
          <table style={S.table}>
            <thead>
      ...
      >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          <div style={S.formGroup}>
            ...
            <UserPicker
              value={opinionForm.reviewer_uuid}
              onChange={(u) => setOpinionForm({ ...opinionForm, reviewer_uuid: u.uuid })}
              placeholder="搜索本人…"
            />
          </div>
          <div style={S.formGroup}>
            ...
            <select
              style={{ ...S.select, width: '100%' }}
              value={opinionForm.role_name}
              onChange={(e) => setOpinionForm({ ...opinionForm, role_name: e.target.value })}
            >
              <option value="">— 选择 —</option>
              {reviewers.map((r: any, i: number) => (
                <option key={i} value={r.role_name}>
                  {r.role_name}
                  {r.submitted_at > 0 ? ' (已提交)' : ''}
                </option>
              ))}
            </select>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 8 }}>
          <div style={S.formGroup}>
            ...
            <select
              style={{ ...S.select, width: '100%' }}
              value={opinionForm.conclusion}
              onChange={(e) => setOpinionForm({ ...opinionForm, conclusion: e.target.value })}
            >
              <option value="pass">✅ 通过</option>
              <option value="conditional_pass">⚠️ 有条件通过</option>
              <option value="fail">❌ 不通过</option>
              {(rv.review_type || 'dcp') === 'tr' && <option value="rework">🔧 返工</option>}
            </select>
          </div>
          <div style={S.formGroup}>
            ...
            <select
              style={{ ...S.select, width: '100%' }}
              value={opinionForm.risk_level}
              onChange={(e) => setOpinionForm({ ...opinionForm, risk_level: e.target.value })}
            >
        ...
        <div style={{ ...S.formGroup, marginTop: 8 }}>
          ...
          <textarea
            style={S.textarea}
            rows={4}
            value={opinionForm.opinion_summary}
            onChange={(e) => setOpinionForm({ ...opinionForm, opinion_summary: e.target.value })}
            placeholder="输入评审意见摘要…"
          />
        </div>
        <button style={{ ...S.btn(true), marginTop: 8 }} onClick={submitOpinion}>
          提交评审意见
        </button>
      </div>
      ...
      <div style={S.card}>
        <h4 style={S.sectionTitle}>关联工作项（{issues.length}个）</h4>
        {issues.length === 0 ? (
          <div style={{ color: '#999', padding: 12, textAlign: 'center', fontSize: 13 }}>
            暂无关联工作项
          </div>
        ) : (
          <div style={S.tableWrap}>
        ...
        {Object.keys(grouped).map((rn) => {
          ...
          return (
            <div key={rn} style={{ fontSize: 12 }}>
              ...
              <span style={{ color: pass === total ? '#52c41a' : '#faad14' }}>
                {pass}/{total}
              </span>
            </div>
          )
        ...
        return (
          <div key={rn} style={{ marginBottom: 4 }}>
            ...
            >
              ...
              <span>{rn}</span>
              <span style={{ color: pass === total ? '#52c41a' : '#faad14', marginLeft: 'auto' }}>
                {pass}/{total}
              </span>
            </div>
            {isExpanded &&
              items
                ...
                .map((item: any) => {
                  ...
                  return (
                    ...
                    >
                      <span style={{ color: statusColor[s], fontSize: 12 }}>
                        {statusDisplay[s]}
                      </span>
...
const IPDFlowChart: React.FC<{
  ...
  if (!layout?.stages || layout.stages.length === 0) return null
  ...
  function roundedPoly(pts: { x: number; y: number }[], radius = 1): string {
    ...
    pts.forEach((corner, i) => {
      ...
      if (i === 0) segs.push(`M ${after.x} ${after.y}`)
        ...
        segs.push(`L ${before.x} ${before.y}`)
        segs.push(`Q ${corner.x} ${corner.y} ${after.x} ${after.y}`)
      }
    })
    const first = pts[0],
  ...
  function stagePath(idx: number): string {
    ...
    if (shape === 'taper' && idx === 0) {
      leftH = HEIGHTS[0]
      rightH = HEIGHTS[1]
    } else if (shape === 'taper' && idx === 1) {
      leftH = HEIGHTS[1]
      rightH = HEIGHTS[2]
    } else {
      leftH = HEIGHTS[2]
  ...
  function stageBottomY(idx: number): number {
    return (
      bandCenterY +
      (stages[idx].shape === 'taper' && idx === 0
        ? HEIGHTS[0]
        : stages[idx].shape === 'taper' && idx === 1
          ? HEIGHTS[1]
          : HEIGHTS[2]) /
        2
    )
  ...
  function getStageStatus(stageCode: string): 'passed' | 'active' | 'rejected' | 'pending' {
    ...
    const dcpMs = markers.filter(
      (m: any) => m.stage === stageCode && (m.reviewType || 'dcp') === 'dcp',
    )
          ...
          .map((m: any, i: number) => {
            ...
            return (
              <g key={`ipd-${i}`}>
                <line
                  x1={x}
                  y1={DCP_DIAMOND_Y + DIAMOND_S}
                  x2={x}
                  y2={topY}
                  stroke={color}
                  strokeWidth="1.5"
                  strokeDasharray={status === 'pending' ? '3,3' : 'none'}
                  opacity={status === 'pending' ? 0.35 : 0.65}
                />
                <line
                  x1={x}
                  y1={bottomY}
                  ...
                  strokeWidth="1"
                  strokeDasharray={status === 'pending' ? '3,3' : 'none'}
                  opacity={status === 'pending' ? 0.2 : 0.35}
                />
                <text
                  x={x}
                  y={DCP_LABEL_Y}
                  ...
                  fill={color}
                >
                  {ph?.phase_name || m.phaseCode}
                </text>
                <polygon
                  points={diamondPoints(x, DCP_DIAMOND_Y, DIAMOND_S)}
                  fill={color}
                  stroke="#fff"
                  strokeWidth="1"
                  style={{ cursor: cr ? 'pointer' : 'default' }}
                  onClick={() => cr && onPhaseClick(cr.review_uuid)}
                />
              </g>
            )
          })}
        ...
        {markers
          ...
          .map((m: any, i: number) => {
            ...
            return (
              <g key={`tr-${i}`}>
                <polygon
                  points={`${x},${TR_APEX_Y} ${x - TRI_S},${TR_BASE_Y} ${x + TRI_S},${TR_BASE_Y}`}
                  fill={color}
                  stroke="#fff"
                  strokeWidth="1"
                  style={{ cursor: cr ? 'pointer' : 'default' }}
...
const StateTimeline: React.FC<{ data: any; effState: string; roundNo: number }> = ({
  data,
  effState,
  roundNo,
}) => {
  ...
  useEffect(() => {
    ...
    if (uuids.length > 0) {
      api
        .resolveReviewerNames([...new Set(uuids)])
        .then(setNameMap)
        .catch(() => {})
    }
  }, [data.state_history])
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          ...
          <span
            style={{
              ...
              fontWeight: 600,
              color: '#fff',
              background: STATE_TIMELINE_COLORS[effState] || '#999',
            }}
          >
            {STATE_TIMELINE_LABELS[effState] || effState}
          </span>
          <span style={{ fontSize: 13, color: '#666' }}>| 第 {roundNo} 轮</span>
          {available.length > 0 && (
            <span style={{ fontSize: 12, color: '#999' }}>
              可流转至: {available.map((s) => STATE_TIMELINE_LABELS[s] || s).join(' / ')}
            </span>
          )}
        </div>
      </div>
        ...
        {history.length === 0 ? (
          ...
          <div style={{ position: 'relative', paddingLeft: 20 }}>
            ...
            {[...history].reverse().map((entry: any, i: number) => {
              ...
              const fromLabel = entry.from_state
                ? STATE_TIMELINE_LABELS[entry.from_state] || entry.from_state
                : ''
              return (
                <div key={i} style={{ position: 'relative', marginBottom: 16, paddingLeft: 16 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                    <span style={{ fontWeight: 600, color: isLatest ? color : '#333' }}>
                      {label}
                    </span>
                    {fromLabel && (
                      <span style={{ fontSize: 11, color: '#999' }}>← {fromLabel}</span>
                    )}
                    {entry.round_no > 1 && (
                      <span
                        style={{
                  ...
                  <div style={{ fontSize: 12, color: '#999', marginTop: 2 }}>
                    ...
                    {entry.by && entry.by !== 'system' ? (
                      <span> | 操作人: {nameMap[entry.by] || entry.by}</span>
                    ) : (
                      <span> | 系统</span>
                    )}
                  </div>
                  {entry.reason && (
                    <div style={{ fontSize: 12, color: '#666', marginTop: 2 }}>{entry.reason}</div>
                  )}
                </div>
              )
            })}
