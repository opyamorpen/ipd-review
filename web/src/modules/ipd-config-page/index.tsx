...
const S: Record<string, any> = {
  ...
  navItem: (a: boolean): React.CSSProperties => ({
    ...
    fontSize: 13,
    color: a ? '#1677ff' : '#333',
    background: a ? '#e6f4ff' : 'transparent',
    borderRight: a ? '2px solid #1677ff' : '2px solid transparent',
...
const App: React.FC = () => {
  ...
  async function loadConfig(showLoading = true) {
    ...
    try {
      ...
      const normPhase = (p: any) => {
        ...
        const needsSplit = ro.some(
          (x: string) => typeof x === 'string' && (x.includes(',') || x.includes('，')),
        )
        const fixedRo = needsSplit
          ? ro.flatMap((x: string) =>
              x
                .split(/[,，]/)
                .map((s: string) => s.trim())
        ...
        return {
          ...p,
          review_type: p.review_type || 'dcp',
          dependencies: p.dependencies || '[]',
          resolution_options: JSON.stringify(fixedRo),
        }
      }
      ...
      if (data.materials?.length)
        setMaterials(
          data.materials.map((m: any) => ({ ...m, review_type: m.review_type || 'dcp' })),
        )
      if (data.indicators?.length)
        setIndicators(
          data.indicators.map((i: any) => ({ ...i, review_type: i.review_type || 'dcp' })),
        )
      if (data.roles?.length)
        setRoles(data.roles.map((r: any) => ({ ...r, review_type: r.review_type || 'dcp' })))
      if (data.checklistItems?.length)
        setChecklistItems(
          data.checklistItems.map((c: any) => ({ ...c, review_type: c.review_type || 'dcp' })),
        )
  ...
  async function handleSave() {
    ...
    setMessage('')
    try {
      if (phases.every((p) => !p.phase_name)) {
        setMessage('至少需要填一个阶段名称')
        setSaving(false)
        return
      }
      ...
      if (reviewIssueType.trim() && !reviewIssueTypeUuid) {
        setMessage('评审单工作项类型必须从团队已有工作项类型中选择')
        setSaving(false)
        return
      }
      if (reviewStatusMapText.trim()) {
        try {
          JSON.parse(reviewStatusMapText)
        } catch {
          setMessage('状态映射必须是合法 JSON（如 {"draft":"状态UUID","reviewing":"状态UUID"}）')
          setSaving(false)
          return
        }
      }
      if (issueTransportText.trim()) {
        try {
          JSON.parse(issueTransportText)
        } catch {
          setMessage('流转传输配置必须是合法 JSON')
          setSaving(false)
          return
        }
      }
      const normPhase = (arr: any[]) =>
        arr.map((x) => ({
          ...x,
          review_type: x.review_type || 'dcp',
          dependencies: x.dependencies || '[]',
        }))
      const normType = (arr: any[]) =>
        arr.map((x) => ({ ...x, review_type: x.review_type || 'dcp' }))
      const body = {
        phases: normPhase(phases.filter((p) => p.phase_name)),
        materials: normType(materials),
        indicators: normType(indicators),
        roles: normType(roles),
  ...
  if (loading)
    return <div style={{ padding: 24, textAlign: 'center', color: '#999' }}>加载配置…</div>
    ...
    <div style={S.container}>
      <div style={S.nav}>
        {NAV.map((item) => (
          <div
            key={item.key}
            style={S.navItem(nav === item.key)}
            onClick={() => {
              setNav(item.key)
              if (item.key === 'profiles') setEditing(false)
            }}
          >
            {item.label}
          </div>
      ...
      <div style={S.content}>
        ...
        {nav === 'phases' && (
          <PhaseTemplates
            phases={phases.filter((p: any) => (p.review_type || 'dcp') === reviewType)}
            allPhases={phases}
            onChange={(v) => {
              const other = phases.filter((p: any) => (p.review_type || 'dcp') !== reviewType)
              setPhases([...other, ...v.map((x: any) => ({ ...x, review_type: reviewType }))])
            }}
            editing={editing}
        ...
        {nav === 'materials' && (
          <MaterialTemplates
            items={materials.filter((m: any) => (m.review_type || 'dcp') === reviewType)}
            onChange={(v) => {
              const other = materials.filter((m: any) => (m.review_type || 'dcp') !== reviewType)
              setMaterials([...other, ...v.map((x: any) => ({ ...x, review_type: reviewType }))])
            }}
            editing={editing}
            phaseObjs={phases
              .filter((p: any) => (p.review_type || 'dcp') === reviewType)
              .map((p: any) => ({ code: p.phase_code, name: p.phase_name || p.phase_code }))}
          />
        )}
        {nav === 'indicators' && (
          <IndicatorTemplates
            items={indicators.filter((i: any) => (i.review_type || 'dcp') === reviewType)}
            onChange={(v) => {
              const other = indicators.filter((i: any) => (i.review_type || 'dcp') !== reviewType)
              setIndicators([...other, ...v.map((x: any) => ({ ...x, review_type: reviewType }))])
            }}
            editing={editing}
            phaseObjs={phases
              .filter((p: any) => (p.review_type || 'dcp') === reviewType)
        ...
        {nav === 'roles' && (
          <RoleRules
            items={roles.filter((r: any) => (r.review_type || 'dcp') === reviewType)}
            onChange={(v) => {
              const other = roles.filter((r: any) => (r.review_type || 'dcp') !== reviewType)
              setRoles([...other, ...v.map((x: any) => ({ ...x, review_type: reviewType }))])
            }}
            editing={editing}
          />
        )}
        {nav === 'checklist' && (
          <ChecklistConfigPanel
            items={checklistItems.filter((c: any) => (c.review_type || 'dcp') === reviewType)}
            roles={roles.filter((r: any) => (r.review_type || 'dcp') === reviewType)}
            onChange={(v) => {
              const other = checklistItems.filter(
                (c: any) => (c.review_type || 'dcp') !== reviewType,
              )
              setChecklistItems([
                ...other,
                ...v.map((x: any) => ({ ...x, review_type: reviewType })),
              ])
            }}
            editing={editing}
            phases={phases.filter((p: any) => (p.review_type || 'dcp') === reviewType)}
        ...
        {nav === 'ipdflow' && (
          <IpdFlowLayoutConfig
            ...
            phases={phases}
            onChange={setIpdFlowLayout}
            editing={editing}
          />
        )}
        {nav === 'notify' && (
          <NotifySettings config={notifyConfig} onChange={setNotifyConfig} editing={editing} />
        )}
        {nav === 'recall' && (
          <RecallSettings config={recallConfig} onChange={setRecallConfig} editing={editing} />
        )}
        {nav === 'remediation' && (
          <RemediationSettings
            ...
            onChange={(name, uuid) => {
              setRemediationIssueType(name)
              setRemediationIssueTypeUuid(uuid)
            }}
            editing={editing}
          />
        )}
        {nav === 'issueMap' && (
          <IssueMappingSettings
            ...
            onChange={(patch: any) => {
              if (patch.type !== undefined) {
                ...
                setReviewIssueTypeUuid(patch.typeUuid || '')
              }
        ...
        {nav === 'profiles' && (
          <ReviewerProfilesPanel
            profiles={profiles.filter((p: any) => (p.review_type || 'dcp') === reviewType)}
            projectBindings={projectBindings.filter(
              (b: any) => (b.review_type || 'dcp') === reviewType,
            )}
            roles={roles.filter((r: any) => (r.review_type || 'dcp') === reviewType)}
            reviewType={reviewType}
            onRefresh={() => loadConfig(false)}
          />
        )}
      </div>
      {editing && nav !== 'profiles' && (
        <div style={S.saveBar}>
          ...
          {message && (
            <span style={{ fontSize: 13, color: message.startsWith('') ? '#ff4d4f' : '#52c41a' }}>
              {message}
            </span>
          )}
        </div>
...
// 节点模板（动态可增删）
// ============================================================
const PhaseTemplates: React.FC<{
  phases: any[]
  allPhases: any[]
  onChange: (v: any[]) => void
  editing: boolean
  reviewType: string
}> = ({ phases, allPhases, onChange, editing, reviewType }) => {
  ...
  function add() {
    ...
    onChange([
      ...phases,
      {
        phase_code: nextCode,
        phase_name: '',
        resolution_options: JSON.stringify(RESOLUTION_PRESETS.slice(0, 3)),
        resolution_template: '',
        dependencies: '[]',
  ...
  return (
    <div>
      <div style={S.sectionTitle}>
        <span>
          {reviewType === 'tr' ? 'TR' : 'DCP'} 节点配置（{phases.length}个节点）
        </span>
        {editing && (
          ...
          return (
            <div key={i} style={{ ...S.card, marginBottom: 12, background: '#fafafa' }}>
              ...
              >
                ...
                {editing && (
                  <button style={S.delBtn} onClick={() => remove(i)}>
                    删除
                  </button>
                )}
              </div>
                  ...
                  {RESOLUTION_PRESETS.map((opt) => {
                    ...
                    return (
                      <span
                        key={opt}
                        style={chipStyle(sel)}
                        onClick={() => editing && toggleArrayItem(i, 'resolution_options', opt)}
                      >
                        {opt}
                      </span>
                    )
                  })}
                  {resOpts.length === 0 && editing && (
                    <span style={{ color: '#ff4d4f', fontSize: 11 }}>至少选1项</span>
                  )}
                </div>
              </div>
              <div style={S.row}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 11, color: '#999', marginBottom: 4 }}>
                    前置阶段的最终决议须为"通过"或"有条件通过"才能发起当前阶段评审
                  </div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center' }}>
                    {allPhases.filter((ap) => ap.phase_code !== p.phase_code && ap.phase_name)
                      .length === 0 ? (
                      <span style={{ fontSize: 12, color: '#bbb' }}>暂无其他已命名阶段可选</span>
                    ) : (
                      allPhases
                        .filter((ap) => ap.phase_code !== p.phase_code)
                        .map((ap) => {
                          ...
                          return (
                            <span
                              ...
                              style={chipStyle(sel)}
                              onClick={() =>
                                editing && toggleArrayItem(i, 'dependencies', ap.phase_code)
                              }
                            >
                              {tag} {ap.phase_name}
                            </span>
...
// 材料模板
// ============================================================
const MaterialTemplates: React.FC<{
  items: any[]
  onChange: (v: any[]) => void
  editing: boolean
  phaseObjs: { code: string; name: string }[]
}> = ({ items, onChange, editing, phaseObjs }) => {
  function add() {
    onChange([
      ...items,
      { material_name: '', applicable_phases: '[]', required: true, sort_order: items.length },
    ])
  }
  function update(idx: number, key: string, value: any) {
    const list = [...items]
  ...
  function updatePhases(idx: number, phs: string[]) {
    update(idx, 'applicable_phases', JSON.stringify(phs))
  }
  function remove(idx: number) {
  ...
  return (
    <div>
      ...
      {items.length === 0 ? (
        <div style={{ color: '#999', padding: 20, textAlign: 'center' }}>暂无材料模板</div>
      ) : (
        items.map((m, i) => (
          <div key={i} style={S.row}>
            <input
              ...
              onChange={(e) => update(i, 'material_name', e.target.value)}
              placeholder="材料名称"
              disabled={!editing}
            />
            <PhaseSelector
              selected={jsonArr(m.applicable_phases)}
              onChange={(v) => updatePhases(i, v)}
              disabled={!editing}
              phases={phaseObjs}
            />
            <label
...
// 指标模板
// ============================================================
const IndicatorTemplates: React.FC<{
  items: any[]
  onChange: (v: any[]) => void
  editing: boolean
  phaseObjs: { code: string; name: string }[]
  ...
  return (
    <div>
      ...
      {items.length === 0 ? (
        <div style={{ color: '#999', padding: 20, textAlign: 'center' }}>暂无指标模板</div>
      ) : (
        items.map((ind, i) => (
          <div key={i} style={S.row}>
            <input
              ...
              onChange={(e) => update(i, 'indicator_name', e.target.value)}
              placeholder="指标名称"
              disabled={!editing}
            />
            <PhaseSelector
              selected={jsonArr(ind.applicable_phases)}
              onChange={(v) => updatePhases(i, v)}
              disabled={!editing}
              phases={phaseObjs}
            />
            ...
            <select
              style={S.select}
              value={ind.threshold_type}
              onChange={(e) => update(i, 'threshold_type', e.target.value)}
              disabled={!editing}
            >
              <option value="达标即通过">达标即通过</option>
              <option value="高于阈值预警">高于阈值预警</option>
              <option value="低于阈值预警">低于阈值预警</option>
            </select>
            <input
              type="number"
              style={{ ...S.input, width: 70 }}
              value={ind.yellow_threshold}
              onChange={(e) => update(i, 'yellow_threshold', parseFloat(e.target.value) || 0)}
              placeholder="黄"
              disabled={!editing}
            />
            <input
              type="number"
              style={{ ...S.input, width: 70 }}
              value={ind.red_threshold}
              onChange={(e) => update(i, 'red_threshold', parseFloat(e.target.value) || 0)}
              placeholder="红"
              disabled={!editing}
            />
            {editing && (
...
// 评审角色
// ============================================================
const RoleRules: React.FC<{ items: any[]; onChange: (v: any[]) => void; editing: boolean }> = ({
  items,
  onChange,
  editing,
}) => {
  function add() {
    onChange([
      ...items,
      { role_name: '', must_vote: true, has_veto: false, sort_order: items.length },
    ])
  }
  function update(idx: number, key: string, value: any) {
    const list = [...items]
  ...
  return (
    <div>
      ...
      <div style={{ color: '#999', fontSize: 12, marginBottom: 12 }}>
        配置评审角色名称及属性。「必投」=该角色必须指定评审人才能发起评审；「否决权」=该角色可否决决议
      </div>
      {items.length === 0 ? (
        <div style={{ color: '#999', padding: 20, textAlign: 'center' }}>
          暂无角色，请点击「+ 添加角色」创建
        ...
        items.map((r, i) => (
          <div key={i} style={{ ...S.row, justifyContent: 'flex-start' }}>
            <input
              style={{ ...S.input, width: 160, fontWeight: 500 }}
              value={r.role_name || ''}
              onChange={(e) => update(i, 'role_name', e.target.value)}
              placeholder="角色名称（如：Chair、TR负责人）"
              disabled={!editing}
            />
            <label
              style={{
            ...
            >
              <input
                type="checkbox"
                checked={r.must_vote}
                onChange={(e) => update(i, 'must_vote', e.target.checked)}
                disabled={!editing}
              />
              必投
            </label>
            ...
            >
              <input
                type="checkbox"
                checked={r.has_veto}
                onChange={(e) => update(i, 'has_veto', e.target.checked)}
                disabled={!editing}
              />
              否决权
            </label>
...
// Checklist 配置面板
// ============================================================
const ChecklistConfigPanel: React.FC<{
  items: any[]
  roles: any[]
  onChange: (v: any[]) => void
  editing: boolean
  phases: any[]
}> = ({ items, roles, onChange, editing, phases }) => {
  const phaseList = phases.map((p: any) => ({
    code: p.phase_code,
    name: p.phase_name || p.phase_code,
  }))
    ...
    <div>
        ...
        {phaseList.map((p, i) => (
          <button
            ...
            style={{
              ...
              border: 'none',
              borderRadius: 4,
              background: phaseIdx === i ? '#1677ff' : '#f0f0f0',
              color: phaseIdx === i ? '#fff' : '#333',
              cursor: 'pointer',
              fontSize: 13,
              fontWeight: phaseIdx === i ? 600 : 400,
            }}
          >
            {p.name} ({items.filter((it: any) => it.phase_code === p.code).length})
          </button>
        ))}
      </div>
              ...
              {roleItems.map((item, idx) => {
                ...
                return (
                  <div key={idx} style={{ ...S.row, justifyContent: 'flex-start', paddingLeft: 8 }}>
                    ...
                    {editing && (
                      <button style={S.delBtn} onClick={() => removeItem(globalIdx)}>
                        ×
                      </button>
                    )}
                  </div>
      ...
      {editing && totalInPhase < 20 && (
        ...
        >
          ...
          />
          <button style={S.btn(true)} onClick={addItem}>
            + 添加
          </button>
        </div>
      )}
...
}> = ({ selected, onChange, disabled, phases }) => {
    ...
    <div ref={ref} style={{ position: 'relative', minWidth: 120 }}>
      ...
      >
        {selectedNames.length > 0 ? (
          ...
          <span style={{ color: '#999' }}>选择阶段</span>
        )}
      </div>
      {open && !disabled && (
        <div
...
const NotifySettings: React.FC<{ config: any; onChange: (c: any) => void; editing: boolean }> = ({
  config,
  onChange,
  editing,
}) => {
    ...
    <div>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        >
          <input
            type="checkbox"
            checked={!!c.on_review_start}
            onChange={() => toggle('on_review_start')}
            disabled={!editing}
          />
          发起评审时通知评审人
        </label>
        ...
        >
          <input
            type="checkbox"
            checked={!!c.on_all_submitted}
            onChange={() => toggle('on_all_submitted')}
            disabled={!editing}
          />
          全体评审人提交后通知决议发布人
        </label>
        ...
        >
          <input
            type="checkbox"
            checked={!!c.on_resolution}
            onChange={() => toggle('on_resolution')}
            disabled={!editing}
          />
          决议发布后通知创建者及评审人
        </label>
        ...
        >
          <input
            type="checkbox"
            checked={c.on_manual_remind !== false}
            onChange={() => toggle('on_manual_remind')}
            disabled={!editing}
          />
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <select
            style={S.select}
            value={String(c.remind_cooldown_seconds || 60)}
            disabled={!editing}
...
// 评审撤回设置
// ============================================================
const RecallSettings: React.FC<{ config: any; onChange: (c: any) => void; editing: boolean }> = ({
  config,
  onChange,
  editing,
}) => {
    ...
    <div>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        >
          <input
            type="checkbox"
            checked={!!c.requireReason}
            onChange={() => toggle('requireReason')}
            disabled={!editing}
          />
          撤回时必须填写原因
        </label>
        ...
        >
          <input
            type="checkbox"
            checked={!!c.clearSubmittedOpinions}
            onChange={() => toggle('clearSubmittedOpinions')}
            disabled={!editing}
          />
      ...
      >
        ⚠ 撤回后评审单回到草稿状态，已提交的评审意见、Checklist
        状态将被清空，评审人待办和决议待办将失效。
      </div>
    </div>
  )
}
...
const RemediationSettings: React.FC<{
  ...
  useEffect(() => {
    ...
    fetch(`/project/api/project/team/${tu}/items/graphql?t=issueTypes`, {
      ...
      body: JSON.stringify({
        query: '{ issueTypes(orderBy: { namePinyin: ASC }) { uuid name } }',
        variables: {},
      }),
    })
      .then((r) => r.json())
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 20, padding: 12, background: '#f9f9f9', borderRadius: 8 }}>
        ...
        {!editing ? (
          <div style={{ ...S.input, background: '#f5f5f5', color: '#666', minHeight: 22 }}>
            {issueType || '（未设置）'}
          </div>
        ) : (
...
// ============================================================
// 工作项映射（新架构：评审单主体 = 系统自定义工作项）
// ============================================================
const REVIEW_STATES: { key: string; label: string }[] = [
  { key: 'draft', label: '草稿' },
  { key: 'ready', label: '就绪' },
  { key: 'reviewing', label: '评审中' },
  { key: 'awaiting_resolution', label: '待决议' },
  { key: 'resolution_published', label: '决议已发布' },
  { key: 'remediation_pending', label: '整改中' },
  { key: 're_reviewing', label: '复审中' },
    ...
    <div>
      ...
      <div style={{ marginBottom: 20, padding: 12, background: '#f9f9f9', borderRadius: 8 }}>
        ...
        <div style={{ color: '#999', fontSize: 12, marginBottom: 8 }}>
          ...
          工作项」创建专用类型（建议命名
          IPD评审单）并启用到目标项目，再在此选择。未配置时无法创建评审单。
        </div>
        {!editing ? (
          <div style={{ ...S.input, background: '#f5f5f5', color: '#666', minHeight: 22 }}>
            {issueType || '（未设置）'}
          ...
          <div ref={ref} style={{ position: 'relative' }}>
            <input
              ...
              value={issueType}
              onChange={(e) => {
                onChange({ type: e.target.value, typeUuid: '' })
                setOpen(true)
              }}
              onFocus={() => setOpen(true)}
              placeholder="输入关键字并从团队工作项类型中选择"
                ...
                {filtered.map((t) => {
                  ...
                  return (
                    <div
                      key={t.uuid}
                      onClick={() => {
                        onChange({ type: t.name, typeUuid: t.uuid })
                        setOpen(false)
                      }}
                      style={{
                        padding: '6px 12px',
      ...
      <div style={{ marginBottom: 12, padding: 12, background: '#f9f9f9', borderRadius: 8 }}>
        ...
        <textarea
          style={{ ...S.input, fontFamily: 'monospace', fontSize: 12, height: 80 }}
          value={transportText}
          disabled={!editing}
          onChange={(e) => onChange({ transport: e.target.value })}
          placeholder="(可选) 留空使用默认内部传输"
        />
      </div>
...
// IPD 流程图布局配置
// ============================================================
const IpdFlowLayoutConfig: React.FC<{
  layout: any
  phases: any[]
  onChange: (v: any) => void
  editing: boolean
}> = ({ layout, phases, onChange, editing }) => {
  ...
  function updateStages(newStages: any[]) {
    onChange({ ...layout, stages: newStages })
  }
  function updateMarkers(newMarkers: any[]) {
    onChange({ ...layout, markers: newMarkers })
  }

  function addStage() {
    updateStages([
      ...stages,
      { code: `stage_${stages.length + 1}`, name: '新阶段', shape: 'rect', widthRatio: 1 },
    ])
  }
  function addMarker() {
    updateMarkers([
      ...markers,
      {
        phaseCode: allPhases[0]?.phase_code || '',
        reviewType: 'dcp',
        stage: stages[0]?.code || '',
        position: 0.5,
        side: 'top',
  ...
  return (
    <div>
      <div style={S.sectionTitle}>
        <span>主阶段带配置（{stages.length}个）</span>
        {editing && (
          <button style={S.addBtn} onClick={addStage}>
            + 添加主阶段
          </button>
        )}
      </div>
      {stages.map((st, i) => (
        <div key={i} style={{ ...S.row, gap: 6, flexWrap: 'wrap' as any }}>
          ...
          <input
            ...
            onChange={(e) => {
              const a = [...stages]
              a[i] = { ...st, widthRatio: parseFloat(e.target.value) || 1 }
              updateStages(a)
            }}
          />
          {editing && (
            <button style={S.delBtn} onClick={() => updateStages(stages.filter((_, j) => j !== i))}>
              ×
            </button>
          )}
        </div>
      ...
      <div style={{ ...S.sectionTitle, marginTop: 20 }}>
        <span>节点挂载配置（{markers.length}个）</span>
        {editing && (
          <button style={S.addBtn} onClick={addMarker}>
            + 添加节点
          </button>
        )}
      </div>
      <div style={{ fontSize: 11, color: '#999', marginBottom: 8 }}>
        phaseCode 必须来自已有节点模板，stage 必须来自上方主阶段。position
        表示节点在所属主阶段中的相对位置，范围 0-1（0=阶段起点，0.5=阶段中点，1=阶段终点）。
      </div>
      {markers.map((m, i) => (
        <div key={i} style={{ ...S.row, gap: 6, flexWrap: 'wrap' as any }}>
          <select
            ...
            onChange={(e) => {
              const a = [...markers]
              a[i] = { ...m, phaseCode: e.target.value }
              updateMarkers(a)
            }}
          >
          ...
          <input
            ...
            onChange={(e) => {
              const a = [...markers]
              a[i] = { ...m, position: parseFloat(e.target.value) || 0 }
              updateMarkers(a)
            }}
          />
          {editing && (
            <button
              style={S.delBtn}
              onClick={() => updateMarkers(markers.filter((_, j) => j !== i))}
            >
              ×
            </button>
          )}
...
const ResolutionRuleConfig: React.FC<{
  rules: any
  roles: any[]
  onChange: (v: any) => void
  editing: boolean
  reviewType: 'dcp' | 'tr'
}> = ({ rules, roles, onChange, editing, reviewType }) => {
  const rule = rules[reviewType] || {
    publisher: { mode: 'single_role', role: '' },
    submitRequirement: { mode: 'must_vote_roles' },
    passRule: {
      mode: reviewType === 'tr' ? 'all_required_submitted' : 'min_approval_count',
      minCount: 3,
      excludeRoles: [],
      approvalConclusions: ['pass', 'conditional_pass'],
      rejectOnAnyVeto: reviewType === 'tr',
    },
    allowedConclusions:
      reviewType === 'tr'
        ? ['pass', 'conditional_pass', 'fail', 'rework']
  ...
  const chipStyle = (sel: boolean): React.CSSProperties => ({
    ...
    fontSize: 12,
    cursor: editing ? 'pointer' : 'default',
    border: `1px solid ${sel ? '#1677ff' : '#d9d9d9'}`,
    background: sel ? '#e6f4ff' : '#fafafa',
    color: sel ? '#1677ff' : '#666',
  })
  ...
  return (
    <div>
      ...
      <div style={{ color: '#999', fontSize: 12, marginBottom: 16 }}>
        配置 {reviewType.toUpperCase()}{' '}
        评审的决议发布权限、提交要求、通过规则和可选决议结果。修改后需保存生效。
      </div>
        ...
        <div>
          {typeRoles.length === 0 && (
            <span style={{ color: '#ff4d4f', fontSize: 12 }}>
              请先在「评审角色」中配置{reviewType.toUpperCase()}角色
            </span>
          )}
          {typeRoles.map((r: any) => {
      ...
      <div style={{ marginBottom: 20, padding: 12, background: '#fafafa', borderRadius: 4 }}>
        ...
        <select
          style={S.select}
          value={rule.submitRequirement?.mode || 'must_vote_roles'}
          disabled={!editing}
      ...
      <div style={{ marginBottom: 20, padding: 12, background: '#fafafa', borderRadius: 4 }}>
        ...
        <select
          style={S.select}
          value={rule.passRule?.mode || 'min_approval_count'}
          disabled={!editing}
          ...
          <>
            ...
            >
              ...
              <input
                ...
                type="number"
                min="1"
                value={rule.passRule?.minCount || 3}
                disabled={!editing}
            ...
            <div style={{ marginTop: 8 }}>
              ...
              <select
                style={S.select}
                value={rule.passRule?.voteScope?.mode || 'must_vote_roles'}
                disabled={!editing}
                ...
                <span>
                  {typeRoles.map((r: any) => (
                    <span
                      key={r.role_name}
                      style={chipStyle(
                        (rule.passRule?.voteScope?.selectedRoles || []).includes(r.role_name),
                      )}
                      onClick={() =>
                        editing && toggleArrayItem('passRule.voteScope.selectedRoles', r.role_name)
                      }
                    >
                      {r.role_name}
                    </span>
              ...
              <span>
                {typeRoles.map((r: any) => (
                  <span
                    key={r.role_name}
                    style={chipStyle(
                      (rule.passRule?.voteScope?.excludeRoles || []).includes(r.role_name),
                    )}
                    onClick={() =>
                      editing && toggleArrayItem('passRule.voteScope.excludeRoles', r.role_name)
                    }
                  >
                    {r.role_name}
                  </span>
            ...
            {(() => {
              ...
              if (mode === 'all_reviewers') scopeCount = typeRoles.length
                ...
                scopeCount = typeRoles.filter((r: any) => selected.includes(r.role_name)).length
              else scopeCount = typeRoles.filter((r: any) => r.must_vote).length
              scopeCount -= typeRoles.filter(
                (r: any) =>
                  exclude.includes(r.role_name) &&
                  (mode === 'all_reviewers' ||
                    (mode === 'must_vote_roles' && r.must_vote) ||
                    (mode === 'selected_roles' && selected.includes(r.role_name))),
              ).length
              ...
              if (minCount > scopeCount)
                return (
                  <div style={{ marginTop: 6, fontSize: 12, color: '#ff4d4f' }}>
                    ⚠ 当前计票范围内最多 {scopeCount} 个角色可计票，不能要求至少 {minCount} 人通过
                  </div>
                )
              return (
                <div style={{ marginTop: 6, fontSize: 12, color: '#52c41a' }}>
                  ✓ 当前计票范围 {scopeCount} 个角色可计票，要求 ≥{minCount} 人通过
                </div>
              )
            })()}
          </>
        ...
        <div style={{ marginTop: 8 }}>
          ...
          >
            <input
              type="checkbox"
              checked={!!rule.passRule?.rejectOnAnyVeto}
              disabled={!editing}
      ...
      <div style={{ marginBottom: 20, padding: 12, background: '#fafafa', borderRadius: 4 }}>
        ...
        <div style={{ color: '#999', fontSize: 12, marginBottom: 8 }}>
          勾选当前{reviewType.toUpperCase()}
          评审允许的最终决议结果。未勾选的选项不会在发布决议时显示。
        </div>
        <div>
          {ALL_CONCLUSIONS.map((c) => (
            <span
              key={c.value}
              style={chipStyle((rule.allowedConclusions || []).includes(c.value))}
              onClick={() => editing && toggleArrayItem('allowedConclusions', c.value)}
            >
              {c.label}
            </span>
          ))}
      ...
      <div style={{ marginBottom: 20, padding: 12, background: '#fafafa', borderRadius: 4 }}>
        ...
        <div style={{ color: '#999', fontSize: 12, marginBottom: 8 }}>
          发布最终决议为「通过」前，强制校验指标红线、Checklist
          完整、前置阶段有效。可设为「阻断通过」「警告放行」「不校验」。旧评审单无此配置时按默认（阻断）执行。
        </div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, alignItems: 'center' }}>
          <label style={{ fontSize: 12, display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            <select
              style={S.select}
              value={rule.gatePolicy?.indicatorRedLine || 'block'}
              disabled={!editing}
          ...
          <label style={{ fontSize: 12, display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            <select
              style={S.select}
              value={rule.gatePolicy?.indicatorGateMode || 'red_only'}
              disabled={!editing}
        ...
        <div
          style={{ display: 'flex', flexWrap: 'wrap', gap: 16, alignItems: 'center', marginTop: 8 }}
        >
          <label style={{ fontSize: 12, display: 'flex', alignItems: 'center', gap: 4 }}>
            Checklist：
            <select
              style={S.select}
              value={rule.gatePolicy?.checklistComplete || 'block'}
              disabled={!editing}
          ...
          <label style={{ fontSize: 12, display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            <select
              style={S.select}
              value={rule.gatePolicy?.checklistScope || 'all'}
              disabled={!editing}
        ...
        >
          <input
            type="checkbox"
            checked={rule.gatePolicy?.prerequisiteRecheck !== false}
            disabled={!editing}
...
const ReviewerProfilesPanel: React.FC<{
  ...
  useEffect(() => {
    ...
    void fetch(`/project/api/project/team/${tu}/items/graphql?t=ipd_projects`, {
      ...
      .then((data) => {
        ...
        const list = (Array.isArray(buckets) ? buckets : []).flatMap((b: any) =>
          Array.isArray(b?.projects) ? b.projects : [],
        )
        ...
        list.forEach((p: any) => {
          ...
          if (uuid)
            byUuid.set(uuid, {
              uuid,
              name: p?.name || p?.identifier || '未命名项目',
              identifier: p?.identifier || p?.key || '',
            })
        })
  ...
  function emptyAssignments(): Record<string, Assignment> {
    ...
    for (const role of roles) {
      out[role.role_name] = {
        mode: role.must_vote || role.has_veto ? 'single' : 'pool',
  ...
  function loadAssignments(profile: any): Record<string, Assignment> {
    ...
    for (const item of raw) {
      ...
      out[item.role_name] = {
        ...
        default_reviewer_uuid:
          item.default_reviewer_uuid || item.default_reviewer || item.reviewer_uuid || '',
        candidate_uuids: Array.isArray(item.candidate_uuids)
          ? item.candidate_uuids.filter(Boolean)
          : item.candidate_uuids_json
            ? jsonArr(item.candidate_uuids_json)
            : [],
      }
    }
    return out
  ...
  async function handleSaveProfile() {
    ...
    try {
      ...
      const role_assignments = roles.map((role: any) => {
        const a = profileForm.assignments[role.role_name] || {
          mode: 'single',
          default_reviewer_uuid: '',
          candidate_uuids: [],
        }
  ...
  function startBinding(profile: any) {
    const bound = projectBindings
      .filter((b) => b.profile_id === profile._key)
      .map((b) => b.project_uuid)
      .filter(Boolean)
      ...
      for (const projectUUID of selected) {
        if (!currentByProject.has(projectUUID)) {
          await upsertProjectBinding({
            ...
            profile_id: bindingProfile._key,
            review_type: reviewType,
          })
        }
      }
      ...
      <div style={{ maxWidth: 1120, margin: '0 auto' }}>
        ...
        >
          <div>
            <div style={{ fontSize: 18, fontWeight: 600, color: '#262626', marginBottom: 6 }}>
              评审人 Profile（{reviewType === 'dcp' ? 'DCP' : 'TR'}）
            </div>
            <div style={{ color: '#8c8c8c', fontSize: 12, lineHeight: 1.6 }}>
              Profile 统一定义各评审角色的默认人选或候选范围，可复用到多个项目。
            </div>
          </div>
          <button style={{ ...S.btn(true), whiteSpace: 'nowrap' }} onClick={startCreate}>
            + 新建 Profile
          </button>
        </div>
        {msg && (
          <div
            style={{
              ...
              fontSize: 12,
              marginBottom: 12,
              background: msg.includes('失败') ? '#fff2f0' : '#f6ffed',
              color: msg.includes('失败') ? '#cf1322' : '#52c41a',
            }}
          >
            {msg}
          </div>
              ...
              {profiles.map((p: any) => {
                ...
                return (
                  <tr key={p._key}>
                    ...
                    <td style={S.td}>
                      <button
                        style={{ ...S.btn(false), padding: '4px 12px', marginRight: 6 }}
                        onClick={() => startEdit(p)}
                      >
                        编辑
                      </button>
                      <button style={S.delBtn} onClick={() => handleDelete(p._key)}>
                        删除
                      </button>
                    </td>
                  </tr>
        ...
        <div style={{ marginTop: 32 }}>
          <div style={{ marginBottom: 12 }}>
            ...
            <div style={{ color: '#8c8c8c', fontSize: 12 }}>
              从 Profile 选择要复用这套评审人配置的项目，保存后会立即显示在对应 Profile 下。
            </div>
          </div>
          <table style={S.table}>
            <thead>
            ...
            <tbody>
              {profiles.map((profile: any) => {
                ...
                const names = bindings
                  .map(
                    (b) =>
                      projects.find((p) => p.uuid === b.project_uuid)?.name ||
                      b.project_name ||
                  ...
                  <tr key={`binding-${profile._key}`}>
                    <td style={S.td}>
                      ...
                      <div style={{ color: '#999', fontSize: 11, marginTop: 3 }}>
                        {(profile.review_type || reviewType).toUpperCase()}
                      </div>
                    </td>
                    <td style={S.td}>
                      {names.length ? (
                        ...
                        <span style={{ color: '#999' }}>尚未应用到项目</span>
                      )}
                    </td>
                    <td style={S.td}>
                      <button
                        style={{ ...S.btn(false), padding: '5px 12px' }}
                        onClick={() => startBinding(profile)}
                      >
                        配置项目
                      </button>
                    </td>
        ...
        {bindingProfile && (
          <div
            ...
            onMouseDown={(e) => {
              if (e.target === e.currentTarget && !bindingSaving) setBindingProfile(null)
            }}
          >
            <div
              style={{
            ...
            >
              ...
              >
                <div style={{ fontSize: 16, fontWeight: 600 }}>
                  配置项目：{bindingProfile.profile_name}
                </div>
                <button
                  title="关闭"
                  style={{
                  ...
                  onClick={() => !bindingSaving && setBindingProfile(null)}
                >
                  ×
                </button>
              </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 18 }}>
                ...
                <button
                  style={S.btn(true, bindingSaving)}
                  disabled={bindingSaving}
    ...
    <div style={{ maxWidth: 1040, margin: '0 auto' }}>
      ...
      >
        <div>
          <div style={{ fontSize: 18, fontWeight: 600, marginBottom: 6 }}>
            {editingProfile._key === 'new'
              ? '新建 Profile'
              : `编辑 Profile：${editingProfile.profile_name}`}
          </div>
          <div style={{ color: '#8c8c8c', fontSize: 12 }}>
            为每个评审角色设置固定默认人选，或者限制可选的候选人范围。
          </div>
        </div>
        <button style={S.btn(false)} onClick={() => setEditingProfile(null)}>
          返回列表
        </button>
      </div>
      {msg && (
      ...
      >
        ...
        <div>
          ...
          <input
            ...
            placeholder="说明这套 Profile 的适用场景"
          />
        </div>
      </div>
      <div style={{ marginBottom: 18 }}>
        <div style={{ marginBottom: 10 }}>
          ...
          <div style={{ color: '#8c8c8c', fontSize: 12 }}>
            单人默认会自动带入并锁定人选；候选池允许发起人从预设范围中选择。
          </div>
        </div>
        <table style={S.table}>
          <thead>
          ...
          <tbody>
            {roles.map((role: any, roleIndex: number) => {
              const ass = profileForm.assignments[role.role_name] || {
                mode: 'single',
                default_reviewer_uuid: '',
                candidate_uuids: [],
              }
              ...
              return (
                <tr key={role.role_name}>
                  ...
                  <td style={S.td}>
                    <select
                      style={{ ...S.select, width: '100%', height: 34 }}
                      value={ass.mode}
                      onChange={(e) =>
                        updateAssignment(role.role_name, {
                          mode: e.target.value as 'single' | 'pool',
                        })
                      }
                    >
                  ...
                  <td style={S.td}>
                    {isSingle ? (
                      <UserPicker
                        ...
                        displayName={
                          members.find((m) => m.uuid === ass.default_reviewer_uuid)?.name
                        }
                        onChange={(u) =>
                          updateAssignment(role.role_name, { default_reviewer_uuid: u.uuid })
                        }
                        placeholder="搜索默认评审人…"
      ...
      <div style={{ display: 'flex', gap: 10, paddingTop: 2 }}>
        <button style={S.btn(true, saving)} disabled={saving} onClick={handleSaveProfile}>
    ...
    <div ref={ref} style={{ position: 'relative' }}>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 6 }}>
        {selectedUuids.length === 0 ? (
          <span style={{ color: '#999', fontSize: 12 }}>尚未选择候选人</span>
        ) : (
      ...
      <input
        ...
        onFocus={() => {
          if (query.trim()) {
            setDropUp(forceDropUp || shouldDropUp(ref.current, 220))
            setOpen(true)
          }
        }}
        onChange={(e) => {
      ...
      {open && keyword && (
        ...
        >
          {filtered.length === 0 ? (
            <div style={{ padding: '10px 12px', color: '#999' }}>无匹配成员</div>
          ) : (
            filtered.map((m) => (
              <div
                key={m.uuid}
              ...
              >
                ...
                {m.email && (
                  <div style={{ color: '#999', fontSize: 11, marginTop: 2 }}>{m.email}</div>
                )}
              </div>
            ))
          )}
    ...
    <div ref={ref} style={{ position: 'relative' }}>
      <div
        style={{
          ...
          alignItems: 'center',
          gap: 6,
          padding: selected.length ? '5px 8px' : '0 8px',
          border: '1px solid #d9d9d9',
          borderRadius: 4,
          background: '#fff',
        }}
          ...
          return (
            ...
            >
              ...
              <button
                type="button"
                title="移除项目"
                onClick={() => onChange(selected.filter((id) => id !== uuid))}
                style={{
                  border: 'none',
                  padding: 0,
                  background: 'transparent',
        ...
        >
          {filtered.length === 0 ? (
            <div style={{ padding: '10px 12px', color: '#999' }}>
              {projects.length === 0 ? '项目列表加载中或暂无可选项目' : '无匹配项目'}
            </div>
          ) : (
...
const UserPicker: React.FC<{
  ...
  function search(kw: string) {
    ...
    const next = base
      .filter(
        (m) => m.name.toLowerCase().includes(keyword) || m.email.toLowerCase().includes(keyword),
      )
      .slice(0, 20)
    setResults(next)
    setOpen(true)
  ...
  if (shownName) {
    return <SelectedMemberChip name={shownName} onClear={handleClear} />
  }
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      ...
      />
      {loading && (
        <div style={{ position: 'absolute', right: 10, top: 7, fontSize: 12, color: '#999' }}>
          搜索中…
        </div>
      )}
